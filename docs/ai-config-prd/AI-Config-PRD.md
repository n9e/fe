...
| GET | `/api/n9e/ai-agent/{id}` | 获取单个 Agent 详情 |
...
| GET | `/api/n9e/ai-llm-config/{id}` | 获取单个 LLM 配置详情 |
...
![image-20260313151023500](/Users/ning/gopath/src/github.com/n9e/fe-ai/docs/ai-config-prd/image-20260313151023500.png)
...
- 技能分组标题（"内置技能"/"自定义技能"）使用 11px 大写字母样式（`textTransform: 'uppercase'`，`letterSpacing: '0.5px'`），两组之间有 12px 间距
- 每个技能项（字号 13px）显示名称（超长省略），未启用的技能旁显示灰色 "off" `Tag`（`color='default'`，字号 11px）
- 选中项样式：背景 `var(--fc-fill-2)` + **左侧 3px 蓝色边框指示条**（`var(--ant-primary-color)`）+ 名称文字加粗（`fontWeight: 500`）
- 悬停样式：未选中项 hover 时背景变为 `var(--fc-fill-1)`，过渡动画 `all 0.2s ease`
- 首次加载时自动选中第一个技能

**右侧面板（详情区域，padding 20px 28px）**：

未选中技能时显示 `Empty` 空状态（`marginTop: '30%'`），提示："请在左侧选择一个技能"。

选中技能后显示：

| 区域 | 交互说明 |
|------|----------|
| **标题行** | 左侧显示技能名称（`<h3>` 18px 600 weight）+ 内置技能标识（蓝色 `Tag color='blue'`，仅 `is_builtin === 1` 时显示），下方可选描述文字（13px 灰色 `var(--fc-text-2)`）。右侧依次：启用开关（`Switch size='small'`，直接调用 `updateAISkill` 切换状态）、编辑按钮（`Button size='small'`，`EditOutlined` 图标 + "编辑" 文字，点击打开编辑弹窗）、删除按钮（仅自定义技能显示，`Button size='small' danger`，`DeleteOutlined` 图标，`Popconfirm` 确认 "确定删除该技能？"）。标题行下方有 `Divider` 分隔线（`margin: '16px 0 0 0'`） |
| **高级配置区域** | 仅在有 license/compatibility/allowed_tools/metadata 任一字段时显示。使用 `Collapse ghost` 面板**默认折叠**，标题 "ADVANCED CONFIG" 使用 12px 大写样式（`textTransform: 'uppercase'`，`letterSpacing: '0.5px'`，灰色 `var(--fc-text-3)`）。展开后内容为卡片容器（`var(--fc-fill-1)` 背景 + 边框 + 圆角），内部使用 CSS Grid 两列布局（`gridTemplateColumns: 'auto 1fr'`，`columnGap: 20`，`rowGap: 12`）。各字段显示：license 值以 `Tag` 渲染；allowed_tools 按空格拆分为多个 `Tag`（等宽字体）；metadata 以 `Tag` 显示，key 部分使用灰色文字 |
| **Instructions 区域** | 12px 大写样式标题 "INSTRUCTIONS"，右侧视图切换使用 `Radio.Group`（`size='small'`，`optionType='button'`，`buttonStyle='solid'`）：预览模式（`EyeOutlined`）和源码模式（`CodeOutlined`）。预览模式：`Markdown` 组件渲染，浅色容器，最大高度 480px 可滚动。源码模式：`<pre>` 标签，**深色主题**（背景 `#1e293b`，文字 `#e2e8f0`），等宽字体（Monaco, Menlo, Consolas, monospace），字号 13px，行高 20px，`word-break: break-word`，最大高度 480px |
| **资源文件区域** | `ResourceFiles` 子组件，标题使用 12px 大写样式（与 Instructions 标题一致），数据来源于 Skill 详情接口返回的 `files` 字段，见下方详细说明 |

**源码模式效果**：

![Skill 源码模式](./images/07b-skill-source-mode.png)

**数据加载**：选中左侧技能后，调用 `getAISkill(id)` 详情接口获取完整数据（包含 `files` 字段），不再单独调用文件列表接口。上传/删除资源文件后，通过回调重新调用详情接口刷新数据。

**资源文件区域**：
- 标题 "RESOURCE FILES"（12px 大写样式，与 Instructions 标题一致） + 「上传文件」按钮（`Button size='small'`，`UploadOutlined` 图标）
- 点击上传按钮触发隐藏的 `<input type="file" accept=".md,.txt,.json,.yaml,.yml,.csv">`，选择文件后调用 `uploadAISkillFile` 上传，成功提示 "Uploaded"，然后通过 `onRefresh` 回调刷新 Skill 详情
- 文件列表表格（`Table size='small'`，不分页），数据来源于详情接口返回的 `files` 字段：
...
| 字段 | 字段名 | 类型 | 必填 | Placeholder | Tooltip | 说明 |
|------|--------|------|------|-------------|---------|------|
| Skill name | `name` | `Input` 文本输入 | 是 | `my-skill-name` | （无） | 编辑模式下，内置技能名称禁止修改（`disabled`） |
...
**高级配置（`Collapse ghost` 面板，默认折叠）**：

以下字段包含在 `Collapse ghost` 可折叠面板中，面板标题 "高级配置"，使用负边距（`marginLeft/Right: -16px`）对齐外层表单。

| 字段 | 字段名 | 类型 | 必填 | Placeholder | Tooltip | 说明 |
|------|--------|------|------|-------------|---------|------|
| License | `license` | `Input` 文本输入 | 否 | "例如 MIT、Apache-2.0" | （无） | 许可证信息 |
| Compatibility | `compatibility` | `Input` 文本输入 | 否 | "例如 需要 git、docker，需要网络访问" | （无） | 环境兼容性说明 |
| Allowed Tools | `allowed_tools` | `Input` 文本输入 | 否 | "空格分隔的工具列表，例如 Bash(git:*) Read" | （无） | 预授权工具列表，空格分隔 |
| Metadata | `metadata` | 动态键值对列表（`Form.List`） | 否 | Key / Value | （无） | 扩展元数据键值对。表单中以 `metadata_list: [{key, value}]` 数组编辑，提交时转换为 `Record<string, string>` 对象。可通过 "添加元数据" 虚线按钮（`type='dashed'`，`PlusOutlined` 图标）添加新行，每行末尾有 `MinusCircleOutlined` 删除按钮 |

> 以上新增字段（license、compatibility、metadata、allowed_tools）参考 [Agent Skills Specification](https://agentskills.io/specification)。
...
interface AISkill {
  ...
  compatibility?: string;  // 兼容性说明（环境依赖、网络需求等）
  metadata?: Record<string, string>;  // 扩展元数据（如 author、version）
  allowed_tools?: string;  // 预授权工具列表（空格分隔）
  enabled: number;         // 是否启用 (0/1)
  created_at: number;      // 创建时间戳
  created_by: string;      // 创建人
  updated_at: number;      // 更新时间戳
  updated_by: string;      // 更新人
  files?: AISkillFile[];   // 关联的资源文件列表（详情接口返回，不含 content）
}
...
> `license`、`compatibility`、`metadata`、`allowed_tools` 字段参考 [Agent Skills Specification](https://agentskills.io/specification)。导入 `.md` 文件时会从 YAML frontmatter 中解析这些字段（frontmatter 中 `allowed-tools` 映射为 `allowed_tools`）。
...
| GET | `/api/n9e/ai-skill/{id}` | 获取单个 Skill 详情，**包含关联的资源文件列表**（`files` 字段，不含文件 content） |
| POST | `/api/n9e/ai-skills` | 新增 Skill |
| PUT | `/api/n9e/ai-skill/{id}` | 更新指定 Skill |
| DELETE | `/api/n9e/ai-skill/{id}` | 删除指定 Skill（级联删除关联的资源文件） |
| POST | `/api/n9e/ai-skills/import` | 导入 Skill 文件（FormData，字段名 `file`），仅支持 `.md` 格式，支持 YAML frontmatter 解析新字段 |
| POST | `/api/n9e/ai-skill/{skillId}/files` | 上传资源文件（FormData），允许 `.md/.txt/.json/.yaml/.yml/.csv`，单文件最大 2MB，每个 Skill 最多 20 个文件 |
| GET | `/api/n9e/ai-skill-file/{fileId}` | 获取单个资源文件详情（含 content） |
| DELETE | `/api/n9e/ai-skill-file/{fileId}` | 删除资源文件 |

> 注意：资源文件列表已合并到 Skill 详情接口，不再提供独立的文件列表接口。
...
| GET | `/api/n9e/mcp-server/{id}` | 获取单个 MCP Server 详情 |
