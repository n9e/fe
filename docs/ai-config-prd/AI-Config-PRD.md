...
| POST | `/api/n9e/ai-llm-config/{id}/test` | 测试 LLM 连接，请求体：`{id?, api_type, api_url, api_key, model}`。id 为 0 时表示未保存的新配置 |
...
| 字段 | 字段名 | 类型 | 必填 | Placeholder | Tooltip | 说明 |
|------|--------|------|------|-------------|---------|------|
| Skill name | `name` | `Input` 文本输入 | 是 | `my-skill-name` | （无） | 内置技能编辑时该字段禁用（`disabled={isEdit && data?.is_builtin === 1}`） |
