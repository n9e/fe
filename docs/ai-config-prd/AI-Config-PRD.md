...
| **元信息区域** | 仅在有 license/compatibility/allowed_tools/metadata 任一字段时显示。每个字段一行，格式：灰色小字 label + 值。metadata 以 `key: value` 逗号分隔显示 |
...
| **资源文件区域** | `ResourceFiles` 子组件，数据来源于 Skill 详情接口返回的 `files` 字段，见下方详细说明 |
