...
/** 转义 XPath 文本值，正确处理含单引号/双引号的情况。内部使用，不直接导出。 */
...
/** 按 label 文本定位 antd 表单项（`.ant-form-item`）。index 用于同页面多个同名 label 时区分。 */
...
/** 向 antd 表单项中的数字输入框（spinbutton）填入数值。 */
...
/** 设置 antd TimePicker 输入框的时间值。可同时用于表单项或行内时间输入。 */
...
/** 定位紧跟指定文本节点之后的第 index 个 combobox（下拉列表框），填入值后按 Enter 确认。 */
...
/**
 * 通过 Playwright 定位器操作 antd Select 下拉选择器，选择指定选项。
 * 自动定位 Select 根节点、点击触发器、等待下拉菜单、选择选项并等待关闭。
 */
...
/** 通过 AI 点击 InputGroup 内的下拉选择器并选择指定选项。 */
export async function selectAntInputGroupOption(aiTap: AiTap, label: string, optionText: string) {
  await aiTap(`${label} 输入组内，${label} 文本右侧的下拉选择器`);
  await aiTap(`选项：${optionText}`, { deepLocate: true });
}

/** 通过 AI 点击 FormItem 内的下拉选择器并选择指定选项。 */
export async function selectAntFormItemOption(aiTap: AiTap, label: string, optionText: string) {
  await aiTap(`${label} 输入组内，${label} 文本下方的下拉选择器`);
  await aiTap(`选项：${optionText}`, { deepLocate: true });
}
