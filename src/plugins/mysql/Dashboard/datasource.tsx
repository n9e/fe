...
export default async function mysqlQuery(options: IOptions): Promise<Result> {
  const { time, targets, variablesWithOptions, datasourceValue } = options;
  ...
  if (targets && typeof datasourceValue === 'number') {
    _.forEach(targets, (target) => {
      ...
      const queryStr = replaceTemplateVariables(query.query, { variablesWithOptions });
      ...
      if (target.__mode__ === '__expr__') {
        ...
        if (mode === 'timeSeries') {
          batchTimeSeriesParams.push({
            ...
            ds_cate: DatasourceCateEnum.mysql,
            query: {
              ref: target.refId,
              from: start,
              to: end,
