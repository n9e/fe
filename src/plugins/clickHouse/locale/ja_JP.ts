const ja_JP = {
  ...
  query: {
    ...
    sql_tip: 'SQL：データをクエリするSQL文。例：select count() AS cnt, event_time, type from system.query_log GROUP BY type, event_time',
    time_field: '時間フィールド',
    time_field_tip_timeSeries: 'SQLで時間を表すフィールド。このフィールドはデータのクエリ範囲と時系列データの時間として使用されます',
    time_field_tip_raw:
