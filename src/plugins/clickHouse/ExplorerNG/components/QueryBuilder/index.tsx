...
import { getCKIndex, buildSql } from '../../../services';
...
export default function index(props: Props) {
  ...
  const indexDataService = () => {
    if (datasourceValue && database && table) {
      return getCKIndex({ cate: DatasourceCateEnum.ck, datasource_id: datasourceValue, database, table })
        .then((res) => {
          ...
          if (!timeField || !fieldExists) {
            const firstDateField = _.find(res, (item) => isCKDateType(item.type, item.normalized_type))?.field;
            if (firstDateField) {
              form.setFieldsValue({
  ...
  const validIndexData = useMemo(() => {
    return _.filter(indexData, (item) => {
      return item.normalized_type !== 'array';
    });
  }, [indexData]);
            ...
            <Space size={SIZE} wrap>
              ...
              <InputGroupWithFormItem size='small' label={t('builder.group_by')}>
                <Form.Item name='group_by' noStyle>
                  <Select
                    size='small'
                    className='min-w-[160px] ck-query-builder-group-by-select'
                    dropdownClassName='ck-query-builder-popup'
