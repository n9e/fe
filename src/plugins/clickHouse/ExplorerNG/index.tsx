...
import { buildCKFilterFromLogValue } from './utils/queryMode';
...
export default function index(props: Props) {
  ...
  const executeQuery = (force = false) => {
    ...
    setTimeout(() => {
      form.validateFields().then((values) => {
        ...
        const hasTimeMacro = /\$__(?:time|date|dt|fromTime|toTime|unixEpoch)/.test(sqlValue);
        if (typeof force === 'boolean' && !force && queryValues.syntax === 'sql' && !hasTimeMacro) {
  ...
  const handleValueFilter = (params: OnValueFilterParams) => {
    ...
    form.setFieldsValue({
      ...
      query: {
        syntax: 'query',
        query_builder_filter: [...(query.query_builder_filter || []), filter],
      },
    });
    executeQuery();
  };
                ...
                <a
                  onClick={() => {
                    DocumentDrawer({
                      ...
                      type: 'iframe',
                      documentPath: `https://flashcat.cloud/docs/content/flashcat/log/discover/what-is-sql-mode-in-ck-discover/`,
                      anchor: '#2-时间宏',
                    });
                  }}
                />
