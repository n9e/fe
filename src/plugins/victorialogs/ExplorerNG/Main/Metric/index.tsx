...
export default function Metric(props: Props) {
  const { indexData, setExecuteLoading } = props;
  ...
  useEffect(() => {
    if (refreshFlag && queryValues?.vizType === 'table' && serviceParams.current !== 1) {
  ...
  useEffect(() => {
    if (!refreshFlag || !datasourceValue || !queryValues?.range || !_.trim(queryValues?.query) || queryValues?.vizType !== 'timeseries') return;
  ...
  if (queryValues?.vizType === 'table') {
    return !_.isEmpty(tableData?.list) ? (
      ...
      <div className='flex justify-center'>
        <Empty className='ant-empty-normal' image='/image/img_executing.svg' description='加载中' imageStyle={{ height: 80 }} />
      </div>
    ) : (
      <>
        <div className='flex justify-between pb-2'>{vizTypeRadio}</div>
        <div className='flex justify-center'>
          <Empty className='ant-empty-normal' image='/image/img_empty.svg' description='暂无数据' imageStyle={{ height: 80 }} />
        </div>
      </>
    );
  }
