const fr_FR = {
  ...
  quick_query_tip:
    "La requête rapide compose une requête à partir d'un modèle SQL figé : pour un champ A supérieur à 0, il suffit de saisir A > 0. Ce bouton fait aussi passer en mode personnalisé, où le SQL se consulte et se modifie",
  ...
  query: {
    ...
    variable_format_tip:
      'Formats avancés : ${variable:csv}, raw, regex, json, text et sqlstring ; text utilise les libellés des options. LIKE réservé à Doris : ${variable:sql_like_or:column} ou ${variable:sql_like_and:column}.',
    advanced_variable_formats: 'Formats de variables avancés',
    dashboard_variable_tip:
      "Variables de requête Doris : les variables à valeurs multiples sont converties par défaut en une liste SQL séparée par des virgules et entourée de guillemets simples. Par exemple, ${host} devient 'host01', 'host02' et peut être utilisé dans IN (${host}).<1/>Utilisez ${variable:format} pour spécifier le format de sortie ; text renvoie les libellés des options. Les formats LIKE réservés à Doris sont ${host:sql_like_or:description} et ${host:sql_like_and:table.column}.<1/>Consultez <advanced>Formats de variables avancés</advanced> pour tous les formats et exemples.<1/>Vous pouvez utiliser des macros temporelles du backend, telles que $__timeFilter(...), $__timeFrom() et $__timeTo(). Consultez la documentation des <a>Conditions de requête</a> pour en savoir plus.",
    ...
    warn_message_content_1:
      'Cette requête risque de parcourir la table entière. Mesurez son incidence sur les performances du stockage, puis décidez de la lancer ou de revenir y ajouter une macro temporelle.',
    ...
    sql_format: {
      ...
      timeseries_tip:
        'À copier dans Vue par structure de table, mode Courbe temporelle, pour consulter les données, ou à réutiliser dans un tableau de bord pour tracer des courbes à partir de données Doris.',
    ...
    interval_tip:
      "Le réglage de la plage n'agit que si le SQL utilise la macro temporelle $__timeFilter.<br />Le système d'alerte s'appuie sur cette fenêtre pour borner les données parcourues, préservant ainsi la réactivité des alertes et les performances de la base",
    offset: 'Requête différée',
    offset_tip:
      "Décale la requête d'un certain nombre de secondes vers le passé avant de l'exécuter, à la manière de l'offset de PromQL.<br />C'est utile lorsque l'écriture ou l'acheminement des données prend du retard, afin d'éviter les fausses alertes dues à des données arrivées trop tard",
    sql_warning_1:
      "Il est vivement conseillé de borner explicitement la période avec $__timeFilter(champ temporel) dans la clause WHERE, faute de quoi la <b>charge de la base peut s'emballer et les requêtes d'alerte expirer</b>",
    sql_warning_2:
      'Le SQL utilise $__timeGroup : la requête renvoie donc des données pour plusieurs instants. Dans ce cas, <b>le système ne retient que le résultat le plus récent</b>',
    duration: 'Durée',
    count: 'Nombre',
