const pt_BR = {
  ...
  quick_query_tip:
    'A consulta rápida gera o SQL a partir de um modelo fixo: para o campo A maior que zero, basta digitar A > 0. Este botão alterna para o modo personalizado, em que o SQL pode ser visto e editado',
  ...
  query: {
    ...
    variable_format_tip:
      'Formatos avançados: ${variable:csv}, raw, regex, json, text e sqlstring; text usa os rótulos das opções. LIKE exclusivo do Doris: ${variable:sql_like_or:column} ou ${variable:sql_like_and:column}.',
    advanced_variable_formats: 'Formatos avançados de variável',
    dashboard_variable_tip:
      "Variáveis de consulta do Doris: por padrão, variáveis com vários valores são convertidas em uma lista de valores SQL separada por vírgulas e entre aspas simples. Por exemplo, ${host} vira 'host01', 'host02' e pode ser usada em IN (${host}).<1/>Use ${variable:format} para especificar o formato de saída; text retorna os rótulos das opções. Os formatos LIKE exclusivos do Doris são ${host:sql_like_or:description} e ${host:sql_like_and:table.column}.<1/>Veja <advanced>Formatos avançados de variável</advanced> para todos os formatos e exemplos.<1/>Você pode usar macros de tempo do backend, como $__timeFilter(...), $__timeFrom() e $__timeTo(). Veja a documentação de <a>Condições de consulta</a> para detalhes.",
    ...
    warn_message_content_1:
      'Esta consulta pode varrer a tabela inteira. Avalie o impacto no desempenho do armazenamento e decida se prossegue ou se volta e acrescenta uma macro de tempo.',
    ...
    sql_format: {
      ...
      timeseries_tip:
        'Pode ser copiado para a visão da estrutura da tabela, no modo série temporal, para ver os dados, ou usado em um dashboard para plotar séries a partir do Doris.',
    ...
    interval_tip:
      'O intervalo da consulta só tem efeito quando o SQL usa a macro $__timeFilter.<br />O sistema de alertas usa essa janela para limitar os dados varridos, preservando a pontualidade dos alertas e o desempenho do banco',
    offset: 'Consulta com atraso',
    offset_tip:
      'Desloca a consulta alguns segundos para trás antes de executá-la, como o offset da PromQL.<br />Costuma servir para lidar com atraso na gravação ou na rede, evitando alertas falsos por dados que ainda não chegaram',
    sql_warning_1:
      'Recomendamos fortemente usar $__timeFilter(campo_de_tempo) no WHERE para delimitar o intervalo; sem isso podem ocorrer <b>carga anormal no banco e tempo limite nas consultas de alerta</b>',
