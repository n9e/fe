const es_ES = {
  ...
  quick_query_tip:
    'La consulta rápida genera el SQL a partir de una plantilla fija: para el campo A mayor que cero, basta con escribir A > 0. Este botón cambia al modo personalizado, donde puedes ver y editar el SQL',
  ...
  query: {
    ...
    variable_format_tip:
      'Formatos avanzados: ${variable:csv}, raw, regex, json, text y sqlstring; text usa las etiquetas de opción. LIKE exclusivo de Doris: ${variable:sql_like_or:column} o ${variable:sql_like_and:column}.',
    advanced_variable_formats: 'Formatos avanzados de variables',
    dashboard_variable_tip:
      "Variables de consulta de Doris: las variables multivalor se convierten de forma predeterminada en una lista de valores SQL entre comillas simples y separados por comas. Por ejemplo, ${host} se convierte en 'host01', 'host02' y se puede usar en IN (${host}).<1/>Use ${variable:format} para especificar el formato de salida; text devuelve las etiquetas de las opciones. Los formatos LIKE exclusivos de Doris son ${host:sql_like_or:description} y ${host:sql_like_and:table.column}.<1/>Consulte <advanced>Formatos avanzados de variables</advanced> para ver todos los formatos y ejemplos.<1/>Puede usar macros de tiempo del backend, como $__timeFilter(...), $__timeFrom() y $__timeTo(). Consulte la documentación de <a>Condiciones de consulta</a> para obtener más información.",
    ...
    warn_message_content_1:
      'Esta consulta puede recorrer la tabla entera. Valora el impacto en el rendimiento del almacenamiento y decide si continúas o vuelves y añades una macro de tiempo.',
    ...
    sql_format: {
      ...
      timeseries_tip:
        'Puedes copiarlo a la vista de la estructura de la tabla, en modo serie temporal, para ver los datos, o usarlo en un dashboard para dibujar series a partir de Doris.',
    ...
    interval_tip:
      'El intervalo de la consulta solo tiene efecto cuando el SQL usa la macro $__timeFilter.<br />El sistema de alertas usa esa ventana para limitar los datos recorridos, lo que preserva la puntualidad de las alertas y el rendimiento de la base de datos',
    offset: 'Consulta con retardo',
    offset_tip:
      'Desplaza la consulta unos segundos hacia atrás antes de ejecutarla, como el offset de PromQL.<br />Suele servir para lidiar con retrasos en la escritura o en la red, y evitar falsas alertas por datos que aún no han llegado',
    sql_warning_1:
      'Te recomendamos encarecidamente usar $__timeFilter(campo_de_tiempo) en el WHERE para acotar el intervalo; sin ello pueden producirse <b>una carga anómala en la base de datos y tiempos de espera agotados en las consultas de alerta</b>',
    sql_warning_2:
      'El SQL usa $__timeGroup, así que la consulta devuelve varios instantes. En ese caso, <b>el sistema solo tiene en cuenta el resultado del instante más reciente</b>',
    duration: 'Duración',
    count: 'Cantidad',
