const ru_RU = {
  ...
  query: {
    ...
    dashboard_variable_tip: `
      ...
      <1 />
      \${variable_name}: текущее значение переменной дашборда
      <1 />
      Переменные с множественным выбором преобразуются в SQL-список значений в одинарных кавычках через запятую. Например, \${host} превращается в 'host01', 'host02' и может использоваться в IN (\${host}).
      <1 />
      SQL LIKE форматы: \${host:sql_like_or} превращается в host LIKE '%server%' OR host LIKE '%web%', а \${host:sql_like_and} объединяет условия через AND.
      <1 />
      Если поле SQL отличается от имени переменной, укажите его явно: \${host:sql_like_or:description}. Поддерживаются псевдонимы таблиц, например \${host:sql_like_and:t.description}.
      <1 />
      Если целевое поле не указано, используется имя переменной.
      <1 />
      Доступны серверные временные макросы, например $__timeFilter(...), $__timeFrom() и $__timeTo(). Подробнее см. <a>документацию по условиям запроса</a>.
    `,
    variable_sql: 'SQL',
    variable_documentation: 'Документация',
