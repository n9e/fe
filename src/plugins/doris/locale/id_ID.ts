const id_ID = {
  ...
  quick_query_tip:
    'Kueri cepat membuat pernyataan kueri dari templat SQL yang tetap; misalnya untuk field A lebih dari 0, cukup ketik A > 0. Tombol ini juga membawa Anda ke mode khusus, tempat SQL-nya bisa dilihat dan diubah',
  ...
  query: {
    ...
    variable_format_tip:
      'Format variabel lanjutan mencakup ${variable:csv}, raw, regex, json, text, dan sqlstring; text menggunakan label opsi. LIKE khusus Doris: ${variable:sql_like_or:column} atau ${variable:sql_like_and:column}.',
    advanced_variable_formats: 'Format variabel lanjutan',
    dashboard_variable_tip:
      "Variabel kueri Doris: variabel bernilai banyak secara default diubah menjadi daftar nilai SQL yang dipisahkan koma dan dibungkus tanda kutip tunggal. Contohnya, ${host} menjadi 'host01', 'host02' dan dapat digunakan dalam IN (${host}).<1/>Gunakan ${variable:format} untuk menentukan format keluaran; text mengembalikan label opsi. Format LIKE khusus Doris adalah ${host:sql_like_or:description} dan ${host:sql_like_and:table.column}.<1/>Lihat <advanced>Format variabel lanjutan</advanced> untuk semua format dan contoh.<1/>Makro waktu backend seperti $__timeFilter(...), $__timeFrom(), dan $__timeTo() dapat digunakan. Lihat dokumentasi <a>Kondisi kueri</a> untuk detailnya.",
    ...
    warn_message_content_1:
      'Kueri ini berpotensi memindai seluruh tabel. Pertimbangkan dampaknya terhadap performa penyimpanan, lalu putuskan untuk melanjutkan atau kembali menambahkan makro waktu.',
    ...
    editMode: {
      ...
      switch_to_builder_confirm_content:
        'Pernyataan SQL saat ini tidak bisa diubah menjadi pengaturan builder, sehingga suntingan SQL-mu akan hilang setelah berpindah. Lanjutkan?',
    ...
    sql_format: {
      ...
      timeseries_tip:
        'Bisa disalin ke Tampilan struktur tabel -> mode Grafik deret waktu untuk melihat datanya, atau dipakai di dasbor untuk menggambar grafik deret waktu dari data Doris.',
    ...
    interval_tip:
      'Pengaturan rentang kueri hanya berlaku bila makro waktu $__timeFilter dipakai di dalam SQL.<br />Sistem alert memakai jendela waktu itu untuk membatasi data yang dipindai, demi menjaga ketepatan waktu alert dan performa basis data',
    offset: 'Kueri tertunda',
    offset_tip:
      'Kueri dijalankan setelah waktunya digeser mundur sekian detik dari waktu kueri saat ini, mirip offset pada PromQL.<br />Ini kerap dipakai saat penulisan data atau jalur pengirimannya tertunda, agar data yang belum tiba tidak memicu alert palsu',
    sql_warning_1:
      'Sangat disarankan membatasi rentang waktu secara eksplisit dengan $__timeFilter(field waktu) pada klausa WHERE; tanpa itu bisa timbul masalah seperti <b>beban basis data yang tidak wajar dan kueri alert yang melewati batas waktu</b>',
    sql_warning_2:
      'SQL ini memakai $__timeGroup sehingga kuerinya mengembalikan data dari beberapa titik waktu. Dalam hal ini, <b>sistem hanya memakai hasil dari titik waktu terbaru</b>',
    duration: 'Durasi',
    count: 'Jumlah',
