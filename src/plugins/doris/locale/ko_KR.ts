const ko_KR = {
  quick_query: '빠른 조회',
  quick_query_tip:
    '빠른 조회는 정해진 SQL 템플릿으로 쿼리문을 빠르게 만들어 줍니다. 예를 들어 필드 A가 0보다 크다는 조건은 A > 0만 입력하면 됩니다. 이 버튼을 누르면 사용자 지정 모드로 옮겨 가 SQL을 보고 고칠 수 있습니다',
  ...
  query: {
    ...
    variable_format_tip:
      '고급 변수 형식: ${variable:csv}, raw, regex, json, text, sqlstring. text는 옵션 레이블을 출력합니다. Doris 전용 LIKE: ${variable:sql_like_or:column} 또는 ${variable:sql_like_and:column}.',
    advanced_variable_formats: '고급 변수 형식',
    dashboard_variable_tip:
      "Doris 쿼리 변수: 다중 선택 변수는 기본적으로 작은따옴표로 감싼 쉼표 구분 SQL 값 목록으로 변환됩니다. 예를 들어 ${host}는 'host01', 'host02'가 되며 IN (${host})에 사용할 수 있습니다.<1/>${variable:format}으로 출력 형식을 지정합니다. text는 옵션 레이블을 반환합니다. Doris 전용 LIKE: ${host:sql_like_or:description} 또는 ${host:sql_like_and:table.column}.<1/>모든 형식과 예시는 <advanced>고급 변수 형식</advanced>을 참고하세요.<1/>$__timeFilter(...), $__timeFrom(), $__timeTo() 등 백엔드 시간 매크로를 사용할 수 있습니다. 자세한 내용은 <a>쿼리 조건</a> 문서를 참조하세요.",
    ...
    interval: '조회 구간',
    interval_tip:
      '조회 구간 설정은 SQL에서 $__timeFilter 시간 매크로를 쓸 때만 적용됩니다.<br />알림 시스템은 그 시간 창으로 훑을 데이터를 제한해 알림의 신속성과 데이터베이스 성능을 지킵니다',
    offset: '지연 조회',
    offset_tip:
      '현재 조회 시각을 정한 초만큼 앞으로 옮긴 뒤 조회를 실행하며 PromQL의 offset과 비슷합니다.<br />데이터 기록이나 전달이 늦어지는 상황에서 아직 도착하지 않은 데이터 때문에 잘못된 알림이 나가는 것을 막을 때 흔히 씁니다',
    sql_warning_1:
      'WHERE 절에서 $__timeFilter(시간 필드)로 시간 범위를 분명히 제한하기를 강력히 권합니다. 그러지 않으면 <b>데이터베이스 부하가 비정상적으로 커지거나 알림 조회가 시간 초과되는</b> 문제가 생길 수 있습니다',
