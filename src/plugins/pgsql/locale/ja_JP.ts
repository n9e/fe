const ja_JP = {
  ...
  query: {
    ...
    query_placeholder:
      "SQLを入力してクエリを実行します。Shift+Enterで改行します",
    ...
    advancedSettings: {
      ...
      valueKey: "値フィールド",
      valueKey_tip:
        "SQLクエリの結果には通常複数の列が含まれています。これらの列のうち、どの列の値をグラフに表示するかを指定できます",
      valueKey_required: "値フィールドは空にできません",
      labelKey: "ラベルフィールド",
      labelKey_tip:
        "SQLクエリの結果には通常複数の列が含まれています。これらの列のうち、どの列をグラフのラベル情報として使用するかを指定できます",
    },
    schema: "メタ情報",
    document: "使用文書",
    dashboard: {
