...
import RawTable from '@/plus/datasource/aliyunSLS/Explorer/RawTable';
...
function index(props: IProps & ModalWrapProps) {
  ...
  return (
    <Drawer title={t('logs.title')} width={960} placement='right' onClose={destroy} visible={visible}>
      <div style={{ marginBottom: 10 }}>
        <Space>
          <TimeRangePicker dateFormat='YYYY-MM-DD HH:mm:ss' value={range} onChange={setRange} />
          <Input.Group>
            <span className='ant-input-group-addon'>{t('logs.count')}</span>
            <Select
              value={limit}
              onChange={(val) => {
                setLimit(val);
          ...
          <Input.Group>
            <span className='ant-input-group-addon'>{t('logs.filter_fields')}</span>
            <Select
              mode='multiple'
