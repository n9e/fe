    ...
    <CardContainer key={field.key} onClose={onClose}>
        ...
        <Row gutter={8}>
          ...
          {syntax === 'sql' && (
            <Col flex='none'>
              <Segmented
                ...
                value={editMode}
                options={[
                  { label: tES('builder.title'), value: 'builder' },
                  { label: tES('builder.code'), value: 'code' },
                ]}
                onChange={(value) => {
          ...
          {syntax === 'sql' && (
            <Col flex='220px'>
              ...
              >
                <Form.Item {...field} name={[field.name, 'interval']} noStyle initialValue={5}>
      ...
      {syntax === 'sql' ? (
        <>
          ...
          {editMode === 'builder' && query?.sql && (
            <CardContainer className='mb-4 bg-fc-150'>
              <SqlMonacoPreview theme={darkMode ? 'dark' : 'light'} value={query.sql} />
            </CardContainer>
          )}
          {editMode === 'builder' && (
            <Button className='mb-3' disabled={disabled || !datasourceValue} onClick={() => setBuilderModalVisible(true)}>
              {tES('builder.open_builder')}
            </Button>
          )}
          {editMode === 'code' && (
            <Form.Item {...field} name={[field.name, 'sql']} label='SQL' rules={[{ required: true, message: tES('query.sql_required') }]}>
              <SqlMonacoEditor disabled={disabled} maxHeight={200} enableAutocomplete enableFormat theme={darkMode ? 'dark' : 'light'} />
            </Form.Item>
          )}
          {editMode === 'builder' && (
            <SQLBuilderModal
