    ...
    _.forEach(menuList, (item) => {
      if (item.children?.length) {
        _.forEach(item.children, (child) => {
          if (child.type === 'tabs' && child.children?.length) {
            _.forEach(child.children, (tabChild) => {
              newMenus.push({ ...tabChild, parent: item, tabParent: child });
            });
          } else {
        ...
        <div className='p-2 overflow-hidden' onMouseMove={() => setLockMouseHover(false)}>
          ...
          <div className='overflow-auto h-[300px] text-sm' ref={menusRef}>
            {_.map(filteredMenus, (item: QuickMenuItem, idx) => {
              const { parent: { label: parentName, icon: parentIcon } = {}, tabParent: { label: tabParentName } = {} } = item;
              return (
                <div
                  key={item.key}
