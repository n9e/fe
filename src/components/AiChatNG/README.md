...
- 当后端返回的 `response` 中包含 `content_type: "form_select"` 且 `content` 为 JSON 字符串时，组件会尝试解析并渲染表单。
- 当前内置支持字段：
  - `busi_group_id`：业务组下拉选择（antd `Select`）
  - `datasource_id`：数据源下拉选择（antd `Select`）
- 点击“确定”后，组件会复用现有发送消息链路，发送的 `query` 结构为：
  - `query.action.param`：仅包含已出现的字段，如 `{ busi_group_id, datasource_id }`（可能只有一个）
  - `query.content`：按照选择项拼装展示文案（如 `业务组：xxx 数据源：yyy`）
...
<AiChat
  ...
  queryPageFrom={{ url: '/explorer/metric?ids=1' }}
  queryAction={{
    key: 'query_generator',
    param: {
