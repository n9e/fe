const ru_RU = {
  toolbar: {
    current_chat: 'Текущий чат',
    new_chat: 'Новый чат',
    history: 'История чатов',
    share: 'Поделиться',
    share_copied: 'Ссылка скопирована',
    switch_to_drawer: 'Переключить в режим Drawer',
    switch_to_floating: 'Переключить в плавающую панель',
  },
  history: {
    untitled: 'Новый чат',
    today: 'Сегодня',
    yesterday: 'Вчера',
    earlier: 'Ранее',
    unknown_time: '--:--',
    delete_confirm: 'Удалить этот диалог?',
    empty: 'История диалогов пуста',
    search_placeholder: 'Поиск диалогов',
    share: 'Поделиться диалогом',
    rename: 'Переименовать',
    more_actions: 'Другие действия с диалогом',
  },
  nightingale: {
    share_fallback_notice: 'У вас нет доступа к исходной странице, здесь показан только диалог',
    title: 'Nightingale AI',
    new_chat: 'Новый чат',
    sessions: 'Диалоги',
    llm_configs: 'Управление LLM',
    skills: 'Управление навыками',
    mcp_servers: 'Управление MCP',
    ai_task: 'Канал задач',
    collapse_sidebar: 'Свернуть боковую панель AI',
    expand_sidebar: 'Развернуть боковую панель AI',
    welcome_cards: {
      overview: {
        title: 'Быстро познакомиться с Nightingale',
        description: 'За минуту узнайте о продукте и возможностях AI-помощника',
        prompt: 'За минуту расскажи об основных функциях мониторинга Nightingale и о том, чем ты можешь мне помочь',
      },
      alerts: {
        title: 'Проверить мои алерты',
        description: 'Какие правила самые шумные, а какие ни разу не срабатывали?',
        prompt: 'Проверь мои текущие правила алертов: какие срабатывали чаще всего за последние 7 дней, а какие не срабатывали ни разу?',
      },
      create_alert: {
        title: 'Создать алерт одной фразой',
        description: 'Опишите сценарий — я создам PromQL и порог',
        prompt: 'Создай правило алерта: отправлять алерт, если загрузка CPU хоста превышает 80% в течение 5 минут',
      },
    },
  },
  input: {
    placeholder: 'Введите вопрос. Enter для отправки, Shift + Enter для новой строки',
    share_readonly_placeholder: 'Просмотр общего чата (только чтение)',
    send: 'Send',
    stop: 'Stop',
  },
  query: {
    title: 'Запрос',
    copied: 'Запрос скопирован',
    copy: 'Копировать',
    execute: 'Выполнить запрос',
    execute_disabled: 'Колбэк выполнения не передан. Доступно только копирование.',
  },
  action: {
    query_generator: 'Сгенерировать запрос',
  },
  message: {
    generating: 'Думаю...',
    processing: 'Все еще обрабатывается',
    hint: 'Подсказка',
    no_llm_title: 'В текущей среде не настроена большая языковая модель',
    no_llm_content: 'Перейдите на страницу <a>Управление LLM</a>, чтобы добавить конфигурацию большой языковой модели',
    stopped: 'Генерация остановлена',
    request_failed: 'Ошибка запроса',
    cancelled: 'Этот ответ был отменен.',
    retry_later: 'Повторите попытку позже.',
    empty_response: 'Нет содержимого ответа',
    thinking: 'Ход рассуждений',
    unsupported_type: 'Неподдерживаемый тип содержимого: {{type}}',
  },
  form_select: {
    title: 'Пожалуйста, заполните следующую информацию, чтобы продолжить:',
    approval_title: 'Пожалуйста, подтвердите указанное выше действие:',
    busi_group: 'Группа бизнеса',
    datasource: 'Источник данных',
    team: 'Команды',
    skill_scope: 'Видимость',
    placeholder_select: 'Выберите',
    confirm: 'Подтвердить',
  },
  alert_rule: {
    title: 'Правило оповещения',
    copy: 'Копировать',
    copied: 'ID правила скопирован',
    duration_seconds: 'в течение {{seconds}} сек.',
    field: {
      id: 'ID правила',
      name: 'Название правила',
      group: 'Группа бизнеса',
      datasource: 'Источник данных',
      cate: 'Тип источника',
      severity: 'Уровень',
      metric: 'Метрика',
      condition: 'Условие',
      note: 'Текст оповещения',
    },
    severity: {
      critical: 'Critical',
      warning: 'Warning',
      info: 'Info',
    },
  },
  dashboard: {
    title: 'Дашборд',
    copied: 'ID дашборда скопирован',
    field: {
      id: 'ID дашборда',
      name: 'Название',
      group: 'Группа бизнеса',
      datasource: 'Источник данных по умолчанию',
      panels_count: 'Панелей',
      variables_count: 'Переменных',
      tags: 'Теги',
    },
  },
  input_request: {
    title: 'It needs to check something first',
  },
  action_frame: {
    updating_query: 'AI обновляет текущий запрос',
    acting: 'The assistant is working on this page',
    done: 'Done',
    stop: 'Stop',
  },
  page_action: {
    title: 'Page action',
    malformed: 'This page action has no content and cannot run',
    not_run: 'Not run on this page',
    unsupported: 'The current page does not provide the action "{{name}}"',
    done: 'Done',
    failed: 'Failed',
    declined: 'Не применено',
  },
  dock: {
    placeholder_draft: 'Напишите следующий запрос и отправьте после завершения',
    wait_to_send: 'Выполнение продолжается. Черновик сохранён; отправьте его после завершения.',
    close_and_stop: 'Закрыть и остановить',
    new_conversation: 'Новый диалог',
    try: 'Попробуйте',
    key_fill: 'Вставить',
    key_send: 'Отправить',
    prompt_cpu: 'Загрузка CPU хоста',
    prompt_cpu_query: 'Создай запрос для загрузки CPU хоста',
    prompt_memory: 'Память хоста',
    prompt_memory_query: 'Создай запрос для использования памяти хоста',
    prompt_disk: 'Диск хоста',
    prompt_disk_query: 'Создай запрос для использования диска хоста',
    prompt_sql_tables: 'Какие есть таблицы',
    prompt_sql_tables_query: 'Перечисли таблицы в этом источнике данных',
    prompt_sql_per_minute: 'Строк в минуту за последний час',
    prompt_sql_per_minute_query: 'Напиши запрос, считающий строки по минутам за последний час',
    prompt_sql_group: 'Количество по статусу',
    prompt_sql_group_query: 'Напиши запрос, считающий строки с группировкой по статусу',
    prompt_log_errors: 'Ошибки за последний час',
    prompt_log_errors_query: 'Напиши SQL, выбирающий логи с ошибками за последний час',
    prompt_log_per_minute: 'Объём логов по минутам',
    prompt_log_per_minute_query: 'Напиши SQL, считающий логи по минутам за последний час',
    prompt_log_group: 'Количество по сервисам',
    prompt_log_group_query: 'Напиши SQL, считающий логи с группировкой по сервису',
    prompt_search_errors: 'Ошибки за последний час',
    prompt_search_errors_query: 'Найди логи с ошибками за последний час',
    prompt_search_service: 'Логи одного сервиса',
    prompt_search_service_query: 'Найди недавние логи одного сервиса',
    prompt_search_timeout: 'Логи с упоминанием timeout',
    prompt_search_timeout_query: 'Найди логи, где упоминается timeout',
    open: 'Ask AI for a query',
    close: 'Close',
    expand: 'Show',
    collapse: 'Hide',
    idle: 'Nothing yet',
    understanding: 'Создание запроса',
    stopped: 'Stopped',
    failed: 'Ошибка запроса',
    asked: 'Нужно уточнение',
    replied: 'Replied',
    applying: 'Ввод запроса',
    querying: 'Выполнение запроса',
    success: 'Запрос выполнен',
    success_count: 'Найдено серий: {{count}}',
    success_rows: 'Найдено строк: {{count}}',
    empty: 'Запрос выполнен, данных нет',
    changed: 'Страница изменена, запрос не применён',
    undone: 'Изменение отменено',
    undo: 'Отменить это изменение',
    stopped_unchanged: 'Остановлено, изменений нет',
    stopped_filled: 'Остановлено; запрос введён, но не запущен',
    stopped_queried: 'Ожидание остановлено; запрос отправлен',
    turn_failed: 'Ошибка ответа',
    timeout: 'Время ожидания запроса истекло. Повторите попытку',
    placeholder_first: 'Describe the query you want, e.g. "CPU usage per host"',
    placeholder_first_sql: 'Опишите нужный запрос, например «строк в минуту за последний час»',
    placeholder_follow_up: 'Keep going…',
    placeholder_answer: 'Answer it…',
  },
  empty: {
    greeting_prefix: 'Здравствуйте, я',
  },
};

export default ru_RU;
