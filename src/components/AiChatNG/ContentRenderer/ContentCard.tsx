 ...
 *
 * Class `ai-chat-content-card` is the hook for surface-scoped overrides
 * (e.g. `[data-ai-surface='query-dock']` flattens the shadow so dock cards
 * do not stack on the floating sheet). Do not strip the shadow here.
 */
export default function ContentCard(props: IContentCardProps) {
  const { icon, title, children, bodyClassName = 'px-4 py-4' } = props;

  return (
    <div className='ai-chat-content-card rounded-xl border border-fc-200 bg-fc-100 shadow-mf'>
