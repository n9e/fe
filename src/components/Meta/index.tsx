...
export default function Meta(props: Props) {
  ...
  const { datasourceCate, datasourceValue, onTreeNodeClick, hideHeader, allowSelect = true } = props;
    ...
    return new Promise<void>((resolve) => {
      ...
      if (_.split(pos, '-')?.length === 2) {
        ...
        getColumns({
          ...
          query: [
            {
              database: key.split(SEPARATOR)[0],
              table: key.split(SEPARATOR)[1],
            },
          ],
        }).then((res) => {
          setTreeData((origin) =>
  ...
  return (
    <div className='explorer-meta-container-main'>
...
export function MetaModal(props: Props) {
  ...
  return (
    <>
      <Modal
        width={400}
        wrapClassName='explorer-meta-modal'
        bodyStyle={{
          padding: 10,
          height: 500,
        }}
