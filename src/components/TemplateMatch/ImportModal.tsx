 ...
 * 两个 Tab 对应「导入仪表盘模板 / 导入告警规则模板」；模板逐条勾选默认全选；
 * 与集成中心手动导入的全部差别 = 数据源预绑定（上下文从保存流程带过来）。
 *
 * 告警规则按分类下拉切换，一次只展示一组、也只导入这一组：
 * 后端 match 是「组内任一哨兵命中即整组返回」（router_template_match.go），
 * 而 categraf 与 telegraf 的指标名大量重合，三组同时返回并不代表两种采集器都在用。
...
export default function ImportModal(props: Props) {
  ...
  // 已成功导入的仪表盘：弹窗保持打开，靠它防止同一份被点出多个副本
  const [dashImported, setDashImported] = useState<(number | string)[]>([]);
  // 业务组只有一个时直接填上，用户不必为一个没得选的选项点一次
  const [bgid, setBgid] = useState<number | undefined>(() => (busiGroups?.length === 1 ? busiGroups[0].id : undefined));
          ...
          <Tabs.TabPane tab={t('tpl_match.tab_dashboards', { count: dashboards.length })} key='dashboards' disabled={_.isEmpty(dashboards)}>
            ...
            {!_.isEmpty(dashImported) && (
              <Alert
                ...
                message={
                  <span>
                    {t('tpl_match.imported_dashboards', { count: dashImported.length })} {/* 新标签打开：用户可能还要接着导告警，不该把这个弹窗冲掉 */}
            ...
            <ImportForm
              ...
              beforeSubmit={
                <>
                  ...
                  {/* 规则条数不定（Linux 通用组就有 26 条），列表内部滚动，不把弹窗顶出视口 */}
              ...
              onSuccess={() => {
                markDsJourney(datasourceId, 'alert_created_at');
                onImported?.('alert');
              }}
            />
          </Tabs.TabPane>
