...
export const baseDatasourceCategories: DatasourceCategory[] = [
  {
    value: 'prometheus',
    label: 'Prometheus',
    label_en: 'Prometheus',
...
export const getDatasourceCategoryDisplayLabel = (cate: Pick<DatasourceCategory, 'label' | 'label_en'> | undefined, lang?: string) => {
  if (!cate) return '';
  if (lang && lang !== 'zh_CN') {
...
export const allDatasourceCategories = [...baseDatasourceCategories, ...advancedCates];

export const getDatasourceCategoryByValue = (cate: string) => {
  if (cate === 'iotdb') {
    return _.find(baseDatasourceCategories, { value: 'iotdb' });
  }
  return _.find(allDatasourceCategories, { value: cate });
};

export const getAuthorizedDatasourceCategories = (feats, isPlus, filter?: (cate: any) => boolean) => {
  let cates = baseDatasourceCategories;
  if (feats && isPlus) {
    cates = _.filter(feats.plugins, (plugin) => {
      return _.find(allDatasourceCategories, { value: plugin.value });
    });
  }
  if (filter) {
...
export const getDatasourceCategoryGraphPro = (cate: string) => {
  const currentCate = getDatasourceCategoryByValue(cate);
  return currentCate?.graphPro;
};

export const getDatasourceCategoryPrimaryType = (cate: string): 'metric' | 'logging' | undefined => {
  const currentCate = _.find(allDatasourceCategories, { value: cate });
  if (currentCate) {
