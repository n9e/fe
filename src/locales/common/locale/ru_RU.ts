const ru_RU = {
  auth: {
    ...
    '403_admin': 'Администраторы: ',
    '403_back': 'Вернуться назад',
  ...
  link: 'Ссылка',
  content_limit_size_msg: 'Конфигурация не может превышать {{num}} символов',
  host: {
  ...
  datasource: {
    ...
    preview: 'Предпросмотр источника данных',
    empty_modal: {
      title: 'Нет конфигурации источника данных, обратитесь к администратору для добавления источника данных',
      title_with_types:
        'В настоящее время поддерживаются только типы источников данных: {{types}}. Другие типы источников данных пока не поддерживаются. Обратитесь к администратору для добавления источника данных',
  ...
  confirm: {
    ...
    close_without_saving: 'Настройки не сохранены, вы уверены, что хотите закрыть?',
  },
  success: {
  ...
  delete_disable_first: 'Нельзя удалить включенный элемент. Сначала отключите его.',
  ...
  'tpl.batch.help': 'Способ выполнения. По умолчанию 0 - параллельное выполнение, 1 - последовательное выполнение, 2 означает последовательное выполнение группами по 2',
  'tpl.tolerance.help': 'Количество допустимых ошибок. По умолчанию 0 - не допускать ошибки. При достижении указанного количества ошибок выполнение останавливается',
