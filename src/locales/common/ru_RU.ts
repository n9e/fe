const ru_RU = {
  ...
  btn: {
    ...
    submit: 'Отправить',
    operations: 'Действия',
    testAndSave: 'Тестировать и сохранять',
  ...
  confirm: {
    ...
    clone: 'Вы уверены, что хотите клонировать?',
  ...
  severity: {
    '1': 'Уровень критической тревоги (Critical)',
    '2': 'Уровень предупреждения (Warning)',
    '3': 'Уровень информации (Info)',
  },
  ...
  'tpl.last_updator': 'Кем обновлено',
  'tpl.last_updated': 'Время обновления',
  'tpl.account.help': 'Аккаунт выполнения, используйте root с осторожностью, если не хотите представлять операционную систему',
  'tpl.batch.help':
    'Параллелизм, по умолчанию 0, что означает полное параллельное выполнение, 1 означает последовательное выполнение, 2 означает выполнение по 2 хостам каждый раз',
  'tpl.tolerance.help':
    'Сколько хостов можно не выполнить, по умолчанию 0, что означает не допускать ни одного сбоя, как только произойдет сбой, выполнение немедленно останавливается',
  'tpl.timeout.help': 'Время выполнения скрипта на одном хосте, единица измерения - секунды',
  'tpl.pause.help': 'Точка остановки, после выполнения каждого хоста останавливается на некоторое время',
  ...
  'tpl.script.help': 'Содержимое скрипта для выполнения',
