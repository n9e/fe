const en_US = {
  no_busi_group_id: 'This alert did not return a business group ID',
  title: 'Active alerts',
  search_placeholder: 'Multiple keywords separated by spaces',
  hours: {
    6: 'Last 6 hours',
    12: 'Last 12 hours',
    24: 'Last 1 day',
    48: 'Last 2 days',
    72: 'Last 3 days',
    168: 'Last 7 days',
    336: 'Last 14 days',
    720: 'Last 30 days',
    1440: 'Last 60 days',
    2160: 'Last 90 days',
  },
  severity: 'Severity',
  eventType: 'Event type',
  rule_name: 'Rule name & event tags',
  first_trigger_time: 'First triggered',
  trigger_time: 'Triggered',
  shield: 'Mute',
  aggregate_rule: 'Aggregate rule',
  aggregate_rule_name: 'Rule name',
  public: 'Public',
  isPublic: 'Is public',
  prod: 'Type',
  status: 'Status',
  status_1: 'Claimed',
  status_0: 'Unclaimed',
  batch_btn: 'Batch operation',
  batch_claim: 'Batch claim',
  batch_unclaim: 'Batch unclaim',
  claim: 'Claim',
  unclaim: 'Unclaim',
  claimant: 'Claimed by',

  delete_confirm: {
    title: 'Delete alert event',
    content:
      'Usually, alert events are only deleted when you are sure the monitoring data will never be reported again (e.g. the data tags were changed or the host was decommissioned), because the deleted alert events can never recover automatically. Are you sure you want to proceed?',
  },

  detail: {
    title: 'Alert detail',
    card_title: 'Event detail',
    buisness_not_exist: 'This business group has been deleted or you do not have permission to view it',
    rule_name: 'Rule title',
    hash: 'Hash',
    group_name: 'Business group',
    rule_note: 'Rule note',
    cate: 'Data source type',
    datasource_id: 'Data source',
    severity: 'Severity',
    is_recovered: 'Status',
    tags: 'Tags',
    target_note: 'Target note',
    trigger_time: 'Triggered',
    first_trigger_time: 'First triggered',
    last_eval_time: 'Last evaluated',
    trigger_value: 'Last triggered value',
    trigger_value2: 'Triggered value',
    recover_time: 'Recovery time',
    rule_algo: 'Alert type',
    rule_algo_anomaly: 'Anomaly',
    rule_algo_threshold: 'Threshold',
    prom_eval_interval: 'Evaluation interval',
    prom_for_duration: 'For duration',
    notify_channels: 'Receiving channels',
    notify_groups_obj: 'Receiving groups',
    callbacks: 'Callbacks',
    runbook_url: 'Runbook URL',
    notify_rules: 'Notification rules',
    detail_url: 'Detail URL',
    host: {
      trigger: 'Trigger',
    },
    trigger: 'Trigger',
    firemap_ql_label: 'Card',
    northstar_ql_label: 'Indicator',
    event_notify_records: {
      label: 'Notification records',
      view: 'View detail',
      result_title: 'Result',
      alert_rule_notify_records: 'Alert rule notification',
      subscription_rule_notify_records: 'Subscription rule notification',
      channel: 'Channel',
      username: 'Username',
      target: 'Target',
      status: 'Status',
      detail: 'Detail',
      sub_id: 'Rule ID',
      notify_rule_id: 'Notification rule ID',
      alert_aggr_events_link: 'Aggregated events',
    },
    task_tpls: {
      label: 'Self-healing template',
    },
    step_tip: 'Time interval of the series, in seconds',
    query_label: 'Query',
    trigger_label: 'Trigger',
    current_anomaly_time: 'Current anomaly time',
    ai_analysis: 'AI analysis',
    ai_analysis_content: 'Analyze the root cause of this alert event',
    ai_analysis_content_ent: 'Analyze the root cause of alert event (id={{id}})',
  },

  sharing_link: {
    title: 'Generate sharing link',
    allow_anonymous: 'Allow anonymous access without login',
    expire_at: 'Expiration time:',
    no_token: 'Invalid link, no token data',
  },
};
export default en_US;
