...
export default class VerticalResizer extends React.PureComponent<VerticalResizerProps, VerticalResizerState> {
  ...
  render() {
    ...
    let draggerStyle;
    let isDraggingCls = '';
    const { position, rightSide, height } = this.props;
    const { dragPosition } = this.state;
    left = `${position * 100}%`;
    const gripStyle = { left, height };
    ...
    return (
      <div className={`VerticalResizer ${isDraggingCls} ${rightSide ? 'is-flipped' : ''}`} ref={this._setRootElm}>
        <div className='VerticalResizer--gripIcon' style={gripStyle} />
        <div aria-hidden className='VerticalResizer--dragger' onMouseDown={this._dragManager.handleMouseDown} style={draggerStyle} />
      </div>
    );
  }
}
