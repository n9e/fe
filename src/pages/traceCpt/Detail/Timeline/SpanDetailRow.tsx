...
import { Log, TraceSpan, KeyValuePair, Link } from '../../type';
...
type SpanDetailRowProps = {
  ...
  linksGetter: (span: TraceSpan, links: KeyValuePair[], index: number) => Link[];
...
export default class SpanDetailRow extends React.PureComponent<SpanDetailRowProps> {
  ...
  render() {
    const { color, columnDivision, detailState, logItemToggle, logsToggle, processToggle, referencesToggle, warningsToggle, span, tagsToggle, traceStartTime, focusSpan } =
      this.props;
    return (
      <TimelineRow className='detail-row'>
        <TimelineRow.Cell width={columnDivision}>
          ...
          <span>
            <span className='detail-row-expanded-accent' aria-checked='true' onClick={this._detailToggle} role='switch' style={{ borderColor: color }} />
          </span>
        </TimelineRow.Cell>
