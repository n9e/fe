...
import { Span, SpanReference, TraceType } from '../../type';
...
function isSpanHidden(span: Span, childrenAreHidden: Set<string>, spansMap: Map<string, Span | TNil>) {
  ...
  //   parentID = undefined;
  //   references = parent && parent.scope_id;
  // }
...
export default class ScrollManager {
  _trace: TraceType | TNil;
  _scroller: IScroller;
  _accessors: Accessors | TNil;

  constructor(trace: TraceType | TNil, scroller: IScroller) {
    this._trace = trace;
  ...
  _scrollToVisibleSpan(direction: 1 | -1, startRow?: number) {
    ...
    const spansMap: Map<string, Span> = childrenAreHidden ? new Map(spans.map((s) => [s.span_id, s] as [string, Span])) : new Map();
    ...
    for (let i = fullViewSpanIndex + direction; i !== boundary; i += direction) {
      const span = spans[i];
      const { span_id, start_time_unix_nano: spanStartTime } = span;
      const spanDuration = (span.end_time_unix_nano = span.start_time_unix_nano);
      ...
      if (findMatches && !findMatches.has(span_id)) {
