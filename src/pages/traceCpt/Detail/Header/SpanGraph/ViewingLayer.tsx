...
import EUpdateTypes from '../../../utils/EUpdateTypes';
...
export default class ViewingLayer extends React.PureComponent<ViewingLayerProps, ViewingLayerState> {
  ...
  _getMarkers(from: number, to: number, isShift: boolean) {
    ...
    return [
      <rect key='fill' className={`ViewingLayer--draggedShift ${cls}`} x={layout.x} y='0' width={layout.width} height={this.props.height - 2} />,
      <rect key='edge' className={`ViewingLayer--draggedEdge ${cls}`} x={layout.leadingX} y='0' width='1' height={this.props.height - 2} />,
    ];
  }
    ...
    return (
      <div aria-hidden className='ViewingLayer' style={{ height }}>
        {(viewStart !== 0 || viewEnd !== 1) && (
          <Button onClick={this._resetTimeZoomClickHandler} className='ViewingLayer--resetZoom' htmlType='button'>
            Reset Selection
          </Button>
        )}
        <svg
        ...
        >
          {leftInactive > 0 && <rect x={0} y={0} height='100%' width={`${leftInactive}%`} className='ViewingLayer--inactive' />}
          {rightInactive > 0 && <rect x={`${100 - rightInactive}%`} y={0} height='100%' width={`${rightInactive}%`} className='ViewingLayer--inactive' />}
          <GraphTicks numTicks={numTicks} />
          {cursorPosition && <line className='ViewingLayer--cursorGuide' x1={cursorPosition} y1='0' x2={cursorPosition} y2={height - 2} strokeWidth='1' />}
