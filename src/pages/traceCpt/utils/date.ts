...
const quantizeDuration = (duration: number, floatPrecision: number, conversionFactor: number) => toFloatPrecision(duration / conversionFactor, floatPrecision) * conversionFactor;
 ...
 * 183840s => 2d 3h
 *
 * @param {number} duration (in nanoseconds)
 * @return {string} formatted duration
 */
export function formatDuration(duration: number): string {
  const _duration = _round(duration);
  // Drop all units that are too large except the last one
  const [primaryUnit, secondaryUnit] = _dropWhile(UNIT_STEPS, ({ microseconds }, index) => index < UNIT_STEPS.length - 1 && microseconds > _duration);
...
export function timeConversion(microseconds: number) {
  ...
  return `${moment.duration(microseconds / 1000, 'ms').as(timeUnit as unitOfTime.Base)}${convertTimeUnitToShortTerm(timeUnit)}`;
}
