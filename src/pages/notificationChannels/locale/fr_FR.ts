const fr_FR = {
  ...
  ident: 'Type de média',
  ident_tip:
    "Catégorie du média de notification : plusieurs médias DingTalk peuvent ainsi partager le type dingtalk. Le type se saisit librement et ne se limite pas aux entrées de la liste ; c'est par ce champ que médias et modèles de message sont reliés",
  ...
  variable_configuration: {
    ...
    params: {
      ...
      title_tip:
        "Définissez les paramètres propres à ce média, par exemple le jeton d'un bot DingTalk ou une clé d'API. Leurs valeurs se renseignent au moment de choisir le média dans une règle de notification",
  ...
  request_configuration: {
    jsm_alert: 'Configuration JSM Alert',
    discord: 'Configuration Discord',
    slackwebhook: 'Configuration Slack Webhook',
    mattermostwebhook: 'Configuration Mattermost Webhook',
  ...
  smtp_request_config: {
    ...
    from: 'Expéditeur',
    from_tip:
      "Nom d'expéditeur ou alias affiché dans le message, qui aide le destinataire à en reconnaître l'origine. Exemple de format : Flashcat <no-reply@notice.flashcat.cloud>",
    insecure_skip_verify: 'Ignorer la vérification du certificat',
  ...
  jira_request_config: {
    top_tip:
      "Recommandé : créez un compte de service dans admin.atlassian.com → Annuaire → Comptes de service, attribuez-lui le rôle User pour Jira, puis créez un jeton d'API. Les tickets seront signalés par ce compte. Les jetons d'API Jira Cloud expirent au plus tard après 1 an ; les notifications échouent ensuite.",
    site_url: 'URL du site',
    site_url_tip:
      "L'adresse avec laquelle vous ouvrez Jira dans le navigateur, par exemple https://your-domain.atlassian.net, sans /rest/api. Avec un jeton à portée, indiquez aussi l'URL du site : Nightingale la convertit automatiquement en adresse de la passerelle Atlassian.",
    site_url_invalid: 'Doit commencer par http:// ou https://, par exemple https://your-domain.atlassian.net',
    token_type: 'Type de jeton',
    token_type_tip:
      "Choisissez la première option pour les jetons de comptes de service et les jetons créés avec Create API token with scopes : ils doivent passer par la passerelle Atlassian (api.atlassian.com) et le Cloud ID est obtenu automatiquement à partir de l'URL du site. Choisissez la seconde pour les jetons créés avec Create API token, envoyés directement à l'URL du site.",
    token_scoped: "Jeton d'API à portée (y compris comptes de service, recommandé)",
    token_classic: "Jeton d'API classique",
    email: 'E-mail',
    email_tip: "L'e-mail du compte propriétaire du jeton. Pour un compte de service, copiez-le depuis sa page de détails (de la forme xxx@serviceaccount.atlassian.com).",
    api_token: "Jeton d'API",
    api_token_tip:
      "Compte de service : ouvrez Credentials sur la page du compte de service et créez un jeton d'API (accordez read:jira-work et write:jira-work ; read:jira-user est facultatif). Compte personnel : https://id.atlassian.com/manage-profile/security/api-tokens. Le jeton n'est affiché qu'une fois. Vous pouvez référencer les Paramètres de variables avec {{.nom_variable}}. Vérifier les identifiants indique précisément quelle autorisation manque.",
    cloud_id: 'Cloud ID',
    cloud_id_tip:
      "Laissez-le généralement vide : il est obtenu automatiquement à partir de l'URL du site. En cas d'échec, ouvrez admin.atlassian.com et copiez le segment après /s/ dans la barre d'adresse (ce n'est pas l'Organization ID).",
    cloud_id_placeholder: 'Laisser vide pour une récupération automatique',
    proxy_tip: "À renseigner si Jira n'est joignable que via un proxy, par exemple http://127.0.0.1:7890",
    insecure_skip_verify: 'Ignorer la vérification du certificat TLS',
  },
  check: {
    btn: 'Vérifier les identifiants',
    passed: 'Identifiants valides',
  ...
  slackwebhook_request_config: {
    top_tip:
      "Une URL de webhook correspond à un salon Slack : elle se renseigne donc dans la règle de notification et ce média peut publier dans autant de salons que nécessaire. Ces réglages sont des valeurs par défaut partagées par toutes les règles. Obtenez l'URL en créant une app sur https://api.slack.com/apps et en activant Incoming Webhooks.",
    proxy_tip: "À renseigner si Slack n'est joignable que via un proxy, p. ex. http://127.0.0.1:7890",
  },
  mattermostwebhook_request_config: {
    top_tip:
      'Une URL de webhook correspond à un canal Mattermost : elle se renseigne donc dans la règle de notification et ce média peut publier dans autant de canaux que nécessaire. Ces réglages sont des valeurs par défaut partagées par toutes les règles. Un administrateur doit activer Enable incoming webhooks dans System Console → Integrations → Integration Management (activé par défaut).',
    username: 'Nom affiché',
    username_tip: "Remplace le nom de l'expéditeur. Nécessite qu'un administrateur active Enable integrations to override usernames, sinon il est ignoré",
    icon: 'Icône',
    icon_tip:
      "Remplace l'avatar de l'expéditeur : une URL d'image ou un code emoji comme :bell:. Nécessite qu'un administrateur active Enable integrations to override profile picture icons, sinon il est ignoré",
    icon_invalid: "Utilisez une URL d'image commençant par http:// ou https://, ou un code emoji comme :bell:",
    proxy_tip: "À renseigner si Mattermost n'est joignable que via un proxy, p. ex. http://127.0.0.1:7890",
    insecure_skip_verify_tip: "À activer pour un Mattermost auto-hébergé avec un certificat auto-signé. S'applique à toutes les URL de webhook de ce média",
  },
  jsm_alert_request_config: {
    top_tip:
      "Les alertes sont envoyées à Operations de Jira Service Management. La clé d'une intégration API détermine l'équipe destinataire, elle se renseigne donc dans chaque règle de notification ; ce média peut envoyer à autant d'équipes que nécessaire. Créez-la dans JSM : équipe → Integrations → Add integration → API.",
    api_url: "URL de l'API",
    api_url_tip: 'Laissez vide pour utiliser https://api.atlassian.com ; le chemin /jsm/ops/integration/v2/alerts est ajouté automatiquement',
    api_url_invalid: 'Doit commencer par http:// ou https://',
    priority_map: 'Gravité et priorité',
    priority_map_tip:
      "Priorité de l'alerte JSM pour chaque gravité Nightingale. Les priorités JSM sont les mêmes sur tout le site : elles se règlent une fois ici pour toutes les règles ; par défaut S1→P1, S2→P2, S3→P3",
    proxy_tip: "À renseigner si api.atlassian.com n'est joignable que via un proxy, par exemple http://127.0.0.1:7890",
  },
  ...
  test: {
    jsm_title: "Clé d'intégration API JSM",
    jsm_tip:
      "Après enregistrement, la clé se renseigne dans la règle de notification ; saisissez-en une ici pour tester. Une vraie alerte est créée dans l'équipe de cette intégration.",
    jsm_api_key_placeholder: "Clé API de l'intégration",
    jsm_with_recovery: 'Tester aussi la résolution (fermer l’alerte)',
    ...
    webhook_title: 'URL du webhook',
    webhook_tip: "Une fois enregistré, l'URL du webhook se renseigne dans la règle de notification ; saisissez-en une ici uniquement pour le test.",
    ...
    jira_project_placeholder: 'Clé du projet, par exemple OPS',
    jira_issue_type_placeholder: 'Type de ticket, par exemple Bug',
    with_recovery: 'Tester aussi la résolution (commenter et fermer le ticket)',
    btn: 'Tester',
    run: 'Envoyer un test',
    ...
    pagerduty_keys_tip:
      'PagerDuty distribue par integration key. Une fois la configuration enregistrée, vous pourrez les choisir via Service/intégration dans les règles de notification ; saisissez-les ici en attendant, plusieurs étant admises.',
