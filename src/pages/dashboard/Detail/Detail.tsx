...
export default function DetailV2(props: IProps) {
  ...
  const { dashboardDefaultRangeIndex, dashboardSaveMode, perms, groupedDatasourceList, darkMode } = useContext(CommonStateContext);
  ...
  const [editModalVariablecontainerReady, setEditModalVariablecontainerReady] = useState(false);
  ...
  const refresh = async (cbk?: () => void) => {
    ...
    fetchDashboard({
      ...
      .then((res) => {
        ...
        setDashboardMeta({
          ...(dashboardMeta || {}),
          dashboardId: _.toString(res.id), // TODO 为什么要转字符串？
          id: res.id,
    ...
    <PageLayout customArea={<div />}>
          ...
          {dashboard.configs?.mode !== 'iframe' ? (
            <>
              <Panels
                ...
                setEditModalVariablecontainerReady={setEditModalVariablecontainerReady}
              />
            </>
          ) : (
      ...
      <Editor
        ...
        onOK={(values, mode) => {
          ...
          setEditModalVariablecontainerReady(false);
        }}
        onCancel={() => {
          setEditModalVariablecontainerReady(false);
        }}
        ...
        onEditModalVariablecontainerReady={() => {
          setEditModalVariablecontainerReady(true);
        }}
      />
      {/*迁移*/}
      <Modal
