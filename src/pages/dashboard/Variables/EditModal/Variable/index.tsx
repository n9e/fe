...
function EditItem(props: IProps) {
  ...
  const regeStr = Form.useWatch('regex', form);
  const formatedReg = regeStr
    ? formatString(
        regeStr,
  ...
  console.log('data', data, variablesWithOptions);
  ...
  return (
    <Form layout='vertical' autoComplete='off' preserve={false} form={form}>
      ...
      {varType === 'query' && <Query datasourceVars={datasourceVars} variablesWithOptions={variablesWithOptions} formatedReg={formatedReg} />}
      {varType === 'textbox' && <Textbox />}
      {varType === 'custom' && <Custom />}
      {varType === 'constant' && <Constant />}
      {varType === 'datasource' && <Datasource editMode={editMode} formatedReg={formatedReg} />}
      {varType === 'datasourceIdentifier' && <DatasourceIdentifier editMode={editMode} formatedReg={formatedReg} />}
      {varType === 'hostIdent' && <HostIdent formatedReg={formatedReg} />}
      <Form.Item>
        <Space>
          <Button
            type='primary'
