...
interface IProps {
  ...
  onChange: (val: IVariable['value']) => void; // 目前只为了外层更新变量 options
}

export default function Variable(props: IProps) {
  const { t } = useTranslation('dashboard');
  const { item, value, onChange } = props;
  const { name, label, multi, allOption, options, type, hide } = item;
  const [dropdownVisible, setDropdownVisible] = useState(false);
  const [selected, setSelected] = useState<IVariable['value']>(value);
  ...
  useEffect(() => {
    ...
    if ((type === 'query' || type === 'custom') && multi) {
      curValue = Array.isArray(value) ? value : value ? ([value] as string[]) : [];
    }
            ...
            onSelect={(v: string) => {
              ...
              if (multi) {
                ...
                if (v === 'all') {
                  ...
                  setSelected(_.without([...((selected as string[]) || []), v], 'all'));
                }
              } else {
                // 完成选择后清空搜索框
                setSearchValue('');
            ...
            onDeselect={(v: string) => {
              ...
              if (multi) {
                ...
                if (v === 'all') {
                  ...
                  newSelected = _.without(selected as string[], v);
                }
