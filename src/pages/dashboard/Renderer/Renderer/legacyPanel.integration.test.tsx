...
import { legacyEsPanel, legacyPromTimeseries, mixedV4Dashboard } from '@/test/fixtures/legacyDashboards';
import { createMockQueryResponse } from '@/test/fixtures/dashboardQuery';
...
jest.mock('./Main', () => ({ __esModule: true, default: ({ queryResult }: { queryResult: { series: unknown[]; error: string } }) => <div data-testid='dashboard-chart'>{queryResult.error || queryResult.series.length}</div> }));
...
describe('legacy dashboard panel rendering', () => {
  ...
  it.each([
    ...
    [legacyEsPanel, [{ ref_id: 'A', query: { filter_language: 'kql' } }, { ref_id: 'A__value_1', query: { filter_language: 'kql' } }]],
    [mixedV4Dashboard, [{ ref_id: 'A', datasource: { id: 1 } }, { ref_id: 'B', datasource: { id: 2 } }]],
  ...
  it('does not repeat a loaded request when props are unchanged', async () => {
    ...
    try {
      fetchDashboardQueryMock.mockResolvedValue(createMockQueryResponse());
      const panel = dashboardMigrator(legacyPromTimeseries).panels[0] as React.ComponentProps<typeof Renderer>['values'];
      const view = render(<Renderer id='panel-1' isPreview panelWidth={800} time={time} values={panel} annotations={[]} />, { wrapper: createCommonStateWrapper({ datasourceList: [] }) });
      await act(async () => {
        jest.advanceTimersByTime(600);
        await Promise.resolve();
        await Promise.resolve();
