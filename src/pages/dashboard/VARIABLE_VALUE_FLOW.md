...
查询变量和面板 SQL 中的 `$` 语法分为两类，二者都不是用户配置的变量依赖：

| 类型         | 示例                                                               | 前端行为                                                                                       |
| ------------ | ------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------- |
| 前端内置变量 | `$__from`、`$__to`、`$__interval`                                  | 根据当前仪表盘时间范围计算并替换后再发送请求。也支持 `${__from}`、`${__to}`、`${__interval}`。 |
| 后端 SQL 宏  | `$__timeFilter(timestamp)`、`$__timeGroup(timestamp, $__interval)` | 识别为 `$__xxx(...)` 保留语法，不参与“缺失变量”校验，保持原样透传给后端 SQL 宏处理器展开。     |

后端 SQL 宏适用于所有 query 类型变量的数据源，并不限于 Doris。前端不维护具体宏名称白名单，以免后端新增宏后被前端误判为缺失的仪表盘变量；不支持或语法错误的宏由后端返回查询错误。

不带 `$` 的 `__from`、`__to`、`__interval` 只是普通 SQL 文本，前端不会替换。`$__name` 但不带函数调用的写法，若不是已知前端内置变量或已配置的仪表盘变量，仍会被视为缺失依赖。
