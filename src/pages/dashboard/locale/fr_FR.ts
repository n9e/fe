const fr_FR = {
  ...
  sharing_link: {
    ...
    revoked: 'Révoqué',
    anonymous_tip:
      "Pendant sa durée de validité, un lien anonyme permet de consulter ce tableau de bord sans connexion et d'interroger les données des sources qu'il référence : partagez-le avec précaution",
    recommend_tip:
      "L'accès anonyme passe par le lien ci-dessous : pendant sa durée de validité, le tableau de bord s'ouvre sans connexion. Pour une publication durable, choisissez une validité en années",
    unit_hour: 'heures',
    unit_day: 'jours',
    unit_month: 'mois',
    unit_year: 'ans',
  ...
  batch: {
    ...
    import_grafana_tip:
      'Seuls les tableaux de bord reposant sur une source Prometheus sont importables, et uniquement dans la limite des types de graphiques et fonctions pris en charge par Nightingale <a>Signaler un problème</a>',
    ...
    show_time_picker: 'Afficher le sélecteur de temps',
    show_time_picker_tip: 'Certaines versions de Grafana masquent le sélecteur de temps en mode kiosque. Activez cette option pour utiliser le sélecteur de temps de ce système.',
    ...
    custom: {
      ...
      table: {
        ...
        tableLayout: {
          ...
          label_tip:
            "En disposition fixe, la largeur des colonnes est répartie également selon leur nombre et aucune barre de défilement horizontale n'apparaît. En disposition automatique, la largeur maximale d'une colonne est de 150 px, si bien que le contenu peut déborder et faire apparaître une barre de défilement.",
          auto: 'Automatique',
          fixed: 'Fixe',
        },
      ...
      tableNG: {
        ...
        enableRowDetail_tip:
          'Une fois activé, une icône de détail apparaît dans la première colonne. Un clic ouvre à droite un tiroir présentant tous les champs et valeurs de la ligne, avec la possibilité de copier la ligne entière ou un champ.',
        rowDetail: {
        ...
        cellOptions: {
          ...
          wrapText_tip:
            "Une fois activé, le texte des cellules revient à la ligne et la hauteur s'adapte au nombre de lignes ; sur de gros volumes, cela peut peser sur les performances",
          'color-background': {
            mode: {
  ...
  query: {
    ...
    expression_placeholder:
      "Applique des opérations mathématiques à une ou plusieurs requêtes. Référencez-les par ${refId}, c'est-à-dire $A, $B, $C, etc. Somme de deux scalaires : $A + $B > 10",
    ...
    legendTip2:
      "Remplace le nom de la légende ou sert de modèle ; ainsi {{hostname}} est substitué par la valeur de l'étiquette hostname. Cela ne vaut aujourd'hui que pour les données temporelles",
    options: 'Options de requête',
    options_max_data_points: 'Nombre maximal de points',
    options_max_data_points_tip:
      'Nombre maximal de points par série, égal par défaut à la largeur du panneau (240 pour un nouveau panneau). Il sert à calculer le pas : (end − start) / maxDataPoints',
