const es_ES = {
  ...
  sharing_link: {
    ...
    revoked: 'Revocado',
    anonymous_tip:
      'Mientras esté vigente, el enlace anónimo permite ver este dashboard sin iniciar sesión y consultar los datos de los orígenes referenciados. Compártelo con cuidado',
    recommend_tip:
      'El acceso anónimo se hace mediante el enlace de abajo, que durante su vigencia no requiere iniciar sesión. Para dejarlo público mucho tiempo, elige una validez en años',
    unit_hour: 'Horas',
    unit_day: 'Días',
    unit_month: 'Meses',
    unit_year: 'Años',
    ...
    revoke_all_confirm_content:
      'Este dashboard todavía tiene {{num}} enlaces anónimos vigentes. Su validez no depende de la configuración de visibilidad: aun después del cambio seguirían abriendo el dashboard sin iniciar sesión. Al confirmar, se revocarán todos los enlaces de este dashboard y se guardará la configuración. La revocación no se puede deshacer.',
  ...
  batch: {
    ...
    import_grafana_tip:
      'Solo pueden importarse dashboards con orígenes de datos Prometheus y con los tipos de gráfico y funciones que admite Nightingale <a>Informar de un problema</a>',
    ...
    show_time_picker: 'Mostrar selector de tiempo',
    show_time_picker_tip: 'Algunas versiones de Grafana ocultan el selector de tiempo en modo quiosco. Activa esta opción para usar el selector de tiempo de este sistema.',
    ...
    custom: {
      ...
      table: {
        ...
        tableLayout: {
          ...
          label_tip:
            'Con el diseño fijo, las columnas se reparten el ancho por igual y no aparece barra de desplazamiento horizontal. Con el diseño automático, cada columna mide como mucho 150 px y el contenido puede desbordarse, lo que genera desplazamiento horizontal.',
          auto: 'Automático',
          fixed: 'Fijo',
        },
      ...
      tableNG: {
        ...
        enableRowDetail_tip:
          'Cuando se activa, la primera columna muestra un icono de detalles. Al pulsarlo, un panel lateral muestra todos los campos y valores de la fila, con opción de copiar la fila entera o campos sueltos.',
        rowDetail: {
        ...
        cellOptions: {
          ...
          wrapText_tip:
            'Cuando se activa, el texto de la celda se ajusta automáticamente y la altura de la fila se adapta al número de líneas; con muchos datos, esto afecta al rendimiento',
          'color-background': {
            mode: {
              label: 'Modo de color',
              options: {
  ...
  query: {
    ...
    expression_placeholder:
      'Aplica operaciones matemáticas a una o varias consultas. Refiérete a ellas con ${refId}, es decir, $A, $B, $C, etc. Suma de dos escalares: $A + $B > 10',
    ...
    legendTip2:
      'Sustituye o define el nombre que se muestra en la leyenda; por ejemplo, {{hostname}} se reemplaza por el valor de la etiqueta hostname. Por ahora solo se aplica a las series temporales',
  ...
  detail: {
    ...
    fullscreen: {
      parameters: {
        tooltip: `Parámetros de URL integrados:
viewMode=fullscreen: abrir la vista a pantalla completa.
__show_header=true: mostrar el encabezado y el selector de tiempo a pantalla completa.
__show_variables=true: mostrar las variables del dashboard a pantalla completa.
__readonly=true: hacer que la pantalla completa sea de solo lectura; desactiva la edición, el arrastre/cambio de tamaño y los avisos de migración.
__disable_fullscreen_exit=true: desactivar el aviso y la salida con Esc.
__disable_variables=region,host: desactivar los controles de las variables indicadas.
__from=...&__to=...: establecer el intervalo de tiempo global.
__refresh=30: establecer el intervalo de actualización automática en segundos.
__timezone=Asia/Shanghai: establecer la zona horaria.
themeMode=light|dark: establecer el tema.
__variable_value_fixed=true: conservar los valores de variables de la URL sin sustituirlos por el almacenamiento local ni por valores predeterminados.`,
      },
      notification: {
