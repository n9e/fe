const ko_KR = {
  ...
  sharing_link: {
    ...
    revoke_all_confirm_content:
      '이 대시보드에는 아직 유효한 익명 공유 링크가 {{num}}개 있습니다. 링크의 유효성은 공개 설정과 무관하므로 유형을 바꿔도 여전히 로그인 없이 이 대시보드를 열 수 있습니다. 계속하면 이 대시보드의 모든 공유 링크를 폐기하고 설정을 저장하며, 폐기는 되돌릴 수 없습니다.',
  ...
  batch: {
    ...
    show_time_picker: '시간 선택기 표시',
    show_time_picker_tip: '일부 Grafana 버전에서는 키오스크 모드에서 시간 선택기가 표시되지 않습니다. 이 옵션을 켜면 이 시스템의 시간 선택기를 사용할 수 있습니다.',
  ...
  var: {
    ...
    reg_object_tip: '표시 이름과 값이 다른 선택지는 값을 정규식과 비교합니다. 일반 일치는 두 필드를 유지하고, 명명된 그룹 text/value는 해당 필드만 덮어씁니다.',
    multi: '다중 선택',
    ...
    custom: {
      ...
      table: {
        ...
        tableLayout: {
          label: '표 배치',
          label_tip:
            '고정 배치에서는 열 너비가 열 개수에 따라 균등하게 나뉘어 가로 스크롤이 생기지 않습니다. 자동 배치에서는 열의 기본 최대 너비가 150px이라 내용이 넘쳐 가로 스크롤이 생길 수 있습니다.',
          auto: '자동',
          fixed: '고정',
        },
        nowrap: '셀에서 줄 바꾸지 않기',
      ...
      tableNG: {
        ...
        enableRowDetail_tip:
          '켜면 표의 첫 열에 상세 아이콘이 나타납니다. 아이콘을 누르면 오른쪽 서랍에서 그 행의 모든 필드와 값을 볼 수 있고 행 전체나 개별 필드를 복사할 수 있습니다.',
        rowDetail: {
          triggerTip: '행 상세 보기',
          title: '상세',
          tableTab: '표',
