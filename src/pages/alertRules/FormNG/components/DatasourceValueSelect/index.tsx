...
function Query({ idx, names, field, remove, invalidDatasourceIds, datasourceList, disabled, fields }) {
  ...
  return (
    <Row gutter={8}>
      {idx > 0 && (
        <Col flex='none'>
          <div className='n9e-alert-rule-ng-datasource-and'>{t('common:and')}</div>
        </Col>
      )}
      ...
      >
        {(fields, { add, remove }) => (
          <div>
              ...
              <div className='flex justify-between items-center'>
                <Space>
                  <div className='ant-col ant-form-item-label'>
                    <label className='ant-form-item-required'>{t('common:datasource.queries.label')}</label>
                  </div>
                    ...
                    {!_.isEmpty(invalidDatasourceIds) && (
                      <span style={{ color: '#ff4d4f' }}>
                        ...
                        >
                          <Space>
                            <WarningOutlined />
                            <span>{t('invalid_datasource_tip_2')}</span>
                            <a
                              onClick={(e) => {
                                e.preventDefault();
                                fetchDatasourceList();
                ...
                <Space className='mb-2'>
                  <Tooltip title={t('common:datasource.managePageLink')}>
                    <Button
                      ...
                      href={IS_ENT ? '/settings/source/timeseries' : '/datasources'}
                      target='_blank'
                    />
                  </Tooltip>
                  <Button
            ...
            >
              {t('form_ng.add_datasource_filter')}
            </Button>
          </div>
        )}
      </Form.List>
