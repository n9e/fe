const ko_KR = {
  ...
  "empty_guide": {
    "title": "아직 워크플로가 없습니다",
    "doc": "사용 문서 보기",
    "mount_hint": "워크플로는 만들었다고 바로 동작하지 않습니다. 알림 규칙이나 알림 전송 규칙에서 참조해야 실행됩니다"
  },
  "scenario_tips": {
    "title": "워크플로는 다음 세 가지 상황에 알맞습니다",
    "denoise": "잡음 줄이기: 등급이 낮거나 반복되는 알림을 전송 전에 버리거나 억제합니다",
  ...
  "executions": {
    ...
    "trigger_by_notify_rule": "알림 전송 규칙 #{{id}}",
    "empty_guide": {
      "title": "이 기간에는 실행 기록이 없습니다",
      "desc": "알림 규칙이나 알림 전송 규칙이 워크플로를 실행할 때마다 여기에 기록이 남습니다. 위쪽 시간 범위를 넓히거나 필터를 느슨하게 해 보세요."
    }
  },
  "test_modal": {
    "title": {
    ...
    "dropped": "이 단계에서 이벤트가 버려지거나 억제되어 이후 프로세서는 실행되지 않고 알림도 생기지 않습니다",
  ...
  "batch": {
    ...
    "delete_confirm": "선택한 워크플로 {{count}}개를 삭제할까요? 이를 참조하는 알림 규칙과 알림 전송 규칙이 동작을 멈춥니다."
  },
  "relabel_fields": {
    "action": "동작",
    "target_label": "대상 레이블",
  ...
  "processor": {
    ...
    "options_desc": {
      ...
      "inhibit": "더 높은 등급의 활성 알림이 있으면 이 알림을 억제합니다",
  ...
  "saved_guide": {
    "title": "워크플로를 저장했습니다",
    "hint": "아직 동작하지는 않습니다. 알림 전송 규칙에서 이 워크플로를 참조해야 이벤트가 여기를 거칩니다.",
    "to_notify_rule": "알림 전송 규칙에 연결하기",
    "done": "완료"
  },
  "label_enrich": {
    "label_source_type": {
  ...
  "inhibit": {
    "help": "이벤트 억제 프로세서는 어떤 알림이 전송될 때 다른 알림 이벤트의 전송을 막아 알림 수를 줄입니다. 예를 들어 같은 알림 규칙에 P1 등급의 활성 장애가 있으면 P2와 P3 등급 알림을 무시합니다. 자세한 내용은 <a>사용 문서</a>를 보세요",
    "tip1": "<b>새 알림</b>이 다음 조건에 맞고",
    "tip2": "그리고",
    "tip3": "초 안에 다음 조건에 맞는 <b>활성 알림</b>이 있으며",
    "tip4": "<b>새 알림</b>과 <b>활성 알림</b>이 다음 항목에서 같을 때",
    "tip5": "위 조건을 모두 만족하면 이 알림은 억제되어 전송되지 않습니다",
  ...
  "inhibit_qd": {
    "help": "조회 결과로 이벤트를 억제합니다. 알림이 발생하면 아래 데이터 조회를 실행하고, 한 건이라도 결과가 있으면 이번 알림을 억제해 전송하지 않으며, 결과가 없으면 평소대로 전송합니다. 자세한 내용은 <a>사용 문서</a>를 보세요",
