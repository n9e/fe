const ja_JP = {
  ...
  empty_guide: {
    ...
    doc: 'ドキュメントを見る',
    mount_hint: 'ワークフローは単体では有効になりません。アラートルールまたは通知ルールから参照されて初めて実行されます。',
  },
  scenario_tips: {
    title: 'ワークフローは次の3つのシーンに適しています',
    denoise: 'ノイズ低減：低レベルや重複したアラートを通知前に破棄・抑制する',
    enrich: 'エンリッチ：アラートに業務ラベル・AI要約・クエリ結果のコンテキストを付与する',
    dispatch: 'ディスパッチ：アラートをチケット/自動化システムにコールバック、または自己修復スクリプトを実行する',
    more: '詳しく見る',
  },
  trigger_mode: {
    label: 'トリガーモード',
  ...
  executions: {
    ...
    message: '実行メッセージ',
    error_node: '失敗ノード',
    inputs_snapshot: '入力変数スナップショット',
    node_results_parsed_title: 'ノード実行結果',
    event_id: 'イベント ID',
    view_all: 'すべて表示',
    filtered_by: 'ワークフロー：{{name}}',
  ...
  test_modal: {
    ...
    result_success: '成功',
    result_failed: '失敗',
    dropped: 'イベントはこのステップで破棄／抑制されました。後続のプロセッサは実行されず、通知も生成されません。',
  ...
  processor: {
    ...
    add_btn: 'プロセッサーを追加',
    typ: 'タイプ',
    typ_required: 'プロセッサのタイプを選択してください。未選択のプロセッサはすべてのイベントで実行に失敗します',
    help_btn: '使用説明',
    options: {
    ...
    options_desc: {
      relabel: 'イベントラベルを変更／追加／削除',
      event_drop: '条件に応じてイベントを破棄し、以降の処理を止める',
      event_update: 'HTTP APIを呼び出し、レスポンスでイベントを更新',
      callback: 'イベントを外部システム（チケット／自動化）へコールバック',
      ai_summary: 'LLMでイベントの要約を生成',
      label_enrich: '内蔵辞書でイベントにラベルを補完',
      script: 'スクリプトを実行してイベントを処理',
      inhibit: 'より高レベルのアクティブアラートがある場合、この通知を抑制',
      inhibit_qd: 'データクエリ結果に基づいてイベントを抑制',
      annotation_qd: 'データクエリ結果に基づいてイベントに情報を付加',
      event_recover: '自己修復タスクをトリガー',
      alert_shot: 'ダッシュボード／ページを撮影してアラートに添付',
    },
    delete_confirm: 'このプロセッサを削除しますか？',
    switch_type_confirm: 'タイプを切り替えると現在の設定がクリアされます。続けますか？',
    drag_tip: 'ドラッグで並べ替え',
    move_up: '上へ',
    move_down: '下へ',
    copy_tip: 'このプロセッサを複製',
  ...
  form_section: {
    filter: {
      title: '処理範囲',
      desc: 'どのアラートイベントがこのワークフローに入るかを決めます。条件はAND、すべて空なら全イベントに一致します。',
    },
    processor: {
      title: 'プロセッサ',
      desc: 'イベントは上から下へ順にプロセッサを通過します。',
    },
    basic: {
      title: '基本情報',
      desc: 'ワークフローの名前、認可チーム、有効状態。',
    },
  },
  no_filter_warning: 'フィルタ条件が未設定です。このワークフローはすべてのアラートイベントを処理します。',
  section_summary: {
    label_count: 'ラベル条件 {{count}} 件',
    attr_count: '属性条件 {{count}} 件',
    no_filter: '全イベントに一致',
    processor_count: 'プロセッサ {{count}} 件',
    unnamed: '未命名',
    enabled: '有効',
    disabled: '無効',
  },
  name_auto: {
    tip: '名前は上の処理範囲とプロセッサから自動生成されます。いつでも編集できます。',
    all: 'すべてのアラート',
    arrow: '→',
    joiner: '-',
  },
  saved_guide: {
    title: 'ワークフローを保存しました',
    hint: 'まだ有効ではありません。通知ルールからこのワークフローを参照すると、イベントが通過します。',
    to_notify_rule: '通知ルールへ',
    done: '完了',
  },
  label_enrich: {
    label_source_type: {
