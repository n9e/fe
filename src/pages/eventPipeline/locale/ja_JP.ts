const ja_JP = {
  ...
  inputs: {
    label: '事前変数',
    help: '事前変数は、以下のプロセッサで {{$inputs.変数名}} を介して参照できます。たとえば、ident 変数を設定し、プロセッサで {{$inputs.ident}} を介して参照して、スクリプト実行用のマシンを指定できます。',
    add_btn: '変数を追加',
    key: '変数名',
  ...
  ai_summary: {
    ...
    prompt_template_tip: `プロンプトテンプレートはAI分析のコアです。{{$event}}でイベントの各フィールドを参照できます。イベントの詳細な構造は[アラート履歴テーブル](https://flashcat.cloud/docs/content/flashcat-monitor/nightingale-v9/usage/alert-notify/events/alert-history/)を参照してください。まずはデフォルトテンプレートから始めてください。`,
