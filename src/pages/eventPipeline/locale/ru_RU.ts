...
Пример: заполните Бизнес-группа==DefaultBusiGroup, что означает, что только когда атрибут "Бизнес-группа" активного события равен DefaultBusiGroup, он будет отфильтрован для последующего процесса подавления событий`,
  ...
  annotation_qd: {
    help: 'Процессор дополнительных запросов — это способ улучшения оповещений. Когда срабатывает оповещение, он может запрашивать связанную информацию из источника данных, такую как журналы, и прикреплять ее к оповещению. Подробности см. в <a>документации</a>',
    query_configs: 'Запрос данных',
    use_event_datasource: 'Использовать источник данных события оповещения',
    datasource_cate_required: 'Тип источника данных не может быть пустым',
    datasource_ids_required: 'Источник данных не может быть пустым',
    select_alert_event_btn: 'Выбрать событие оповещения',
    select_alert_event_tip: 'Подсказка выбора события оповещения',
    select_alert_event_label: 'Выбранное событие оповещения',
    query_required: 'Условие запроса не может быть пустым',
    ...
    annotation_val_placeholder: 'Содержимое дополнительного поля, поддерживает синтаксис шаблонов, заполняет результаты запроса в виде переменных',
    data_preview: 'Предварительный просмотр данных',
    data_preview_query: 'Выражение запроса',
