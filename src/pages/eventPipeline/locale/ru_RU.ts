const ru_RU = {
  title: 'Рабочий процесс',
  title_add: 'Добавить событие Pipeline',
  title_edit: 'Редактировать событие Pipeline',
  title_clone: 'Клонировать событие Pipeline',
  teams: 'Авторизованные команды',
  teams_tip:
    'Определите, какие члены команды могут просматривать и изменять эту конфигурацию, можно связать несколько команд<br />Например: если конфигурация авторизована для infra-team, то только члены команды infra-team могут получить доступ или изменить эту конфигурацию.',
  basic_configuration: 'Основная конфигурация',
  filter_enable: 'Условия фильтрации',
  label_filters: 'Применимые метки',
  label_filters_tip:
    'Установите условия фильтрации меток для обработки событий. События будут обрабатываться только если они содержат метки, соответствующие этой конфигурации.<br />Пример: введите service=mon, что означает, что событие будет обработано только если оно содержит метку service=mon.',
  attribute_filters: 'Применимые атрибуты',
  attribute_filters_tip:
    'Установите условия фильтрации атрибутов для обработки событий. События будут обрабатываться только если они содержат атрибуты, соответствующие этой конфигурации.<br />Пример: введите Бизнес-группа==DefaultBusiGroup, что означает, что событие будет обработано только если его атрибут "Бизнес-группа" равен DefaultBusiGroup.',
  attribute_filters_value: 'Значение атрибута',
  attribute_filters_options: {
    group_name: 'Бизнес-группа',
    cluster: 'Источник данных',
    is_recovered: 'Это событие восстановления?',
    severity: 'Уровень серьезности',
  },
  use_case: {
    label: 'Назначение',
    firemap: 'Карта пожаротушения',
    event_pipeline: 'Обработка событий',
  },
  processors_col: 'Процессоры',
  clone_suffix: '-копия',
  unsaved_confirm: 'Есть несохранённые изменения. Всё равно закрыть?',
  search_placeholder: 'Поиск по имени, примечанию или типу процессора',
  empty_guide: {
    title: 'Пока нет рабочих процессов',
    doc: 'Открыть документацию',
    mount_hint: 'Рабочий процесс не действует сам по себе. Он выполняется только при ссылке из правила оповещения или правила уведомления.',
  },
  scenario_tips: {
    title: 'Рабочие процессы подходят для трёх сценариев',
    denoise: 'Шумоподавление: отбрасывать или подавлять оповещения низкого уровня и дубли до уведомления',
    enrich: 'Обогащение: добавлять к оповещениям бизнес-метки, AI-резюме или контекст из запросов',
    dispatch: 'Диспетчеризация: отправлять оповещения в системы тикетов/автоматизации или запускать скрипты самовосстановления',
    more: 'Подробнее',
  },
  trigger_mode: {
    label: 'Режим триггера',
    event: 'Триггер события',
    api: 'Триггер API',
  },
  disabled: {
    filter_placeholder: 'Статус',
    form_label: 'Включить',
    label: 'Включить',
    false: 'Включено',
    true: 'Отключено',
  },
  inputs: {
    label: 'Входные переменные',
    help: 'Входные переменные могут быть использованы в процессорах ниже через {{$inputs.имя_переменной}}. Например, вы можете установить переменную ident и в процессоре ссылаться на нее через {{$inputs.ident}}, чтобы указать машину для выполнения скрипта.',
    add_btn: 'Добавить переменную',
    key: 'Имя переменной',
    key_required: 'Имя переменной не может быть пустым',
    value: 'Значение по умолчанию',
    description: 'Описание переменной',
  },
  executions: {
    title: 'История выполнения',
    search_placeholder: 'Введите ключевое слово для поиска',
    status: {
      label: 'Статус',
      running: 'Выполняется',
      success: 'Успешно',
      failed: 'Ошибка',
      terminated: 'Прервано',
      skipped: 'Пропущено',
      streaming: 'Потоковая передача',
    },
    id: 'ID выполнения',
    pipeline_name: 'Имя рабочего процесса',
    mode: 'Режим триггера',
    created_at: 'Время начала',
    finished_at: 'Время окончания',
    duration_ms: 'Длительность (мс)',
    trigger_by: 'Инициатор',
    detail_title: 'Детали выполнения',
    detail_basic_info: 'Основная информация',
    error_message: 'Сообщение об ошибке',
    message: 'Сообщение о выполнении',
    error_node: 'Узел с ошибкой',
    inputs_snapshot: 'Снимок входных переменных',
    node_results_parsed_title: 'Результаты выполнения узлов',
    event_id: 'ID события',
    view_all: 'Показать все',
    filtered_by: 'Рабочий процесс: {{name}}',
    trigger_by_alert_rule: 'Правило оповещения #{{id}}',
    trigger_by_notify_rule: 'Правило уведомления #{{id}}',
    empty_guide: {
      title: 'Нет записей о выполнении за этот период',
      desc: 'Каждый запуск появляется здесь после того, как рабочий процесс запускается правилом оповещения или правилом уведомления. Попробуйте расширить диапазон времени выше или ослабить фильтры.',
    },
  },
  test_modal: {
    title: {
      settings: 'Выберите тестовое событие',
      result: 'Результат тестового запуска',
    },
    result_success: 'Успешно',
    result_failed: 'Ошибка',
    dropped: 'Событие было отброшено / подавлено на этом шаге. Последующие процессоры не выполнятся, уведомление не создаётся.',
    steps_title: 'Результаты по узлам',
    event_preview_title: 'Обработанное событие',
    back_btn: 'Выбрать другое событие',
    back_btn_mock: 'Настроить тестовое событие заново',
    fidelity_note: 'Тестовый запуск использует путь API-триггера и пропускает часть реальных шагов (например, проверку условий фильтра), поэтому результат может не полностью совпадать с реальным оповещением. Проверьте на реальном событии.',
    fidelity_note_mock: 'Тестовый запуск использует путь API-триггера и пропускает часть реальных шагов (например, проверку условий фильтра). В этом запуске использовалось смоделированное событие, а не реальное оповещение, поэтому перед запуском в эксплуатацию проверьте на реальном событии.',
    mode: {
      history: 'Историческое событие',
      mock: 'Тестовое событие',
    },
    mock: {
      desc: 'Тестовое событие создаётся сервером и не сохраняется, поэтому проверить настройку обработчиков можно и без истории оповещений. Уровень и состояние восстановления можно менять, чтобы проверить обработчики, зависящие от них.',
      preview_title: 'Тестовое событие',
      severity: 'Уровень важности',
      is_recovered: 'Событие восстановления',
      tags: 'Метки',
      empty_alert: 'За этот период нет событий тревоги',
      switch_btn: 'Использовать тестовое событие',
    },
  },
  batch: {
    not_select: 'Пожалуйста, сначала выберите рабочие процессы для операции',
    export: {
      title: 'Пакетный экспорт',
    },
    delete: 'Удалить выбранные',
    enable: 'Массовое включение',
    disable: 'Массовое отключение',
    already_enabled: 'Все выбранные рабочие процессы уже включены',
    already_disabled: 'Все выбранные рабочие процессы уже отключены',
    enable_confirm: 'Включить выбранные рабочие процессы ({{count}})?',
    disable_confirm: 'Отключить выбранные рабочие процессы ({{count}})?',
    delete_enabled_confirm: '{{count}} из них ещё включены и будут отключены перед удалением. Продолжить?',
    delete_confirm: 'Удалить выбранные рабочие процессы ({{count}})? Ссылающиеся на них правила оповещения/уведомления перестанут работать.',
  },

  relabel_fields: {
    action: 'Действие',
    target_label: 'Целевая метка',
    replacement: 'Значение',
    source_labels: 'Исходные метки',
    separator: 'Разделитель',
    regex: 'Регулярное выражение',
    replace_hint: 'replace: берёт значение исходных меток, извлекает его регулярным выражением и записывает в целевую метку. Если заполнить только целевую метку и значение, событие получит фиксированную метку. Если целевая метка пуста, обработчик ничего не делает.',
  },
  processor_message: {
    drop_hit: 'Условие совпало — событие отброшено',
    drop_miss: 'Условие не совпало — событие продолжает путь дальше',
    no_change: 'без изменений',
  },
  processor: {
    title: 'Процессор',
    add_btn: 'Добавить процессор',
    typ: 'Тип',
    typ_required: 'Выберите тип процессора: без него процессор будет завершаться с ошибкой на каждом событии',
    help_btn: 'Инструкция по использованию',
    options: {
      relabel: 'Перезапись меток события',
      callback: 'Обратный вызов',
      event_update: 'Обновление события',
      event_drop: 'Отбрасывание события',
      ai_summary: 'AI Резюме',
      label_enrich: 'Обогащение меток события',
      script: 'Обработка скрипта',
      inhibit: 'Подавление событий',
      inhibit_qd: 'Подавление событий по данным запроса',
      annotation_qd: 'Обогащение аннотаций по данным запроса',
      event_recover: 'Восстановление события',
      alert_shot: 'Скриншот предупреждения',
    },
    category: {
      rewrite: 'Изменение',
      denoise: 'Шумоподавление',
      enrich: 'Обогащение',
      dispatch: 'Вызов и запуск',
      other: 'Прочее',
    },
    options_desc: {
      relabel: 'Изменить / добавить / удалить метки события',
      event_drop: 'Отбросить событие по условию и прекратить обработку',
      event_update: 'Вызвать HTTP API и обновить событие ответом',
      callback: 'Отправить событие во внешнюю систему (тикеты / автоматизация)',
      ai_summary: 'Сгенерировать сводку события с помощью LLM',
      label_enrich: 'Обогатить событие метками из встроенного словаря',
      script: 'Обработать событие запуском скрипта',
      inhibit: 'Подавить это уведомление при активном оповещении более высокого уровня',
      inhibit_qd: 'Подавить событие по результату запроса данных',
      annotation_qd: 'Добавить сведения к событию по результату запроса данных',
      event_recover: 'Запустить задачу самовосстановления',
      alert_shot: 'Сделать скриншот дашборда / страницы и приложить к оповещению',
    },
    delete_confirm: 'Удалить этот процессор?',
    switch_type_confirm: 'Смена типа очистит текущую конфигурацию процессора. Продолжить?',
    drag_tip: 'Перетащите для сортировки',
    move_up: 'Вверх',
    move_down: 'Вниз',
    copy_tip: 'Дублировать этот процессор',
  },
  form_section: {
    filter: {
      title: 'Область обработки',
      desc: 'Определяет, какие события оповещений попадают в этот рабочий процесс. Условия объединяются по И; оставьте все пустыми, чтобы соответствовать любому событию.',
    },
    processor: {
      title: 'Процессоры',
      desc: 'Событие проходит через процессоры сверху вниз.',
    },
    basic: {
      title: 'Основные сведения',
      desc: 'Имя, авторизованные команды и состояние рабочего процесса.',
    },
  },
  no_filter_warning: 'Условия фильтра не заданы. Этот рабочий процесс будет обрабатывать все события оповещений.',
  section_summary: {
    label_count: 'условий по меткам: {{count}}',
    attr_count: 'условий по атрибутам: {{count}}',
    no_filter: 'соответствует всем событиям',
    processor_count: 'процессоров: {{count}}',
    unnamed: 'Без имени',
    enabled: 'Включено',
    disabled: 'Отключено',
  },
  name_auto: {
    tip: 'Имя формируется из области обработки и процессоров выше; его можно изменить в любой момент.',
    all: 'Все оповещения',
    arrow: '→',
    joiner: '-',
  },
  saved_guide: {
    title: 'Рабочий процесс сохранён',
    hint: 'Он ещё не активен — сошлитесь на этот рабочий процесс из правила уведомления, чтобы события проходили через него.',
    to_notify_rule: 'К правилам уведомлений',
    done: 'Готово',
  },
  label_enrich: {
    label_source_type: {
      label: 'Источник метки',
      options: {
        built_in_mapping: 'Встроенный словарь меток',
      },
    },
    label_mapping_id: 'Название словаря',
    help: 'Используйте указанные метки в исходных метках для поиска в словаре, добавьте поля, найденные в словаре, в событие тревоги в соответствии с конфигурацией "Новые метки"',
    source_keys: {
      label: 'Исходные метки',
      text: 'Поле <strong>{{field}}</strong> в словаре соответствует метке в событии',
      target_key_placeholder: 'Ключ метки',
      target_key_required: 'Ключ метки не может быть пустым',
    },
    append_keys: {
      label: 'Новые метки',
      source_key_placeholder: 'Поле в словаре',
      rename_key: 'Переименовать ключ метки',
      target_key_placeholder: 'Ключ метки',
    },
  },
  callback: {
    url: 'URL',
    advanced_settings: 'Расширенные настройки',
    basic_auth_user: 'Имя пользователя авторизации',
    basic_auth_user_placeholder: 'Пожалуйста, введите имя пользователя авторизации',
    basic_auth_pass: 'Пароль авторизации',
    basic_auth_pass_placeholder: 'Пожалуйста, введите пароль авторизации',
  },
  event_drop: {
    hint: 'Событие отбрасывается, когда шаблон выводит true; любой другой вывод пропускает его дальше. Доступные переменные: $event.Severity (1/2/3), $event.IsRecovered, $event.RuleName, $event.TagsMap.<метка>',
    snippets_label: 'Вставить пример',
    snippets: {
      severity: 'Отбрасывать оповещения уровня S3 (Info)',
      recovered: 'Отбрасывать уведомления о восстановлении',
      tag: 'Отбрасывать по метке',
      rule_name: 'Отбрасывать по имени правила',
    },
    replace_confirm: 'Текущее условие будет заменено примером. Продолжить?',
    content: 'Логика суждения',
    content_placeholder: 'Используйте go template синтаксис. Если в конце будет true, событие будет отброшено на этом этапе.',
  },
  ai_summary: {
    llm_config: 'Использовать конфигурацию LLM',
    llm_config_placeholder: 'Выберите настроенный LLM (оставьте пустым для ручного ввода параметров ниже)',
    llm_config_tip: 'Используйте существующую конфигурацию модели из «Конфигурация AI - Конфигурации LLM» (её модель/ключ/URL и т. д.). Оставьте пустым, чтобы использовать введённые вручную параметры ниже.',
    url_placeholder: 'Пожалуйста, введите адрес API сервиса',
    url_required: 'Пожалуйста, введите URL',
    api_key_placeholder: 'API ключ',
    api_key_required: 'Пожалуйста, введите API Key',
    model_name: 'Имя модели',
    model_name_placeholder: 'например, deepseek-chat',
    model_name_required: 'Пожалуйста, введите имя модели',
    prompt_template: 'Шаблон запроса',
    prompt_template_required: 'Пожалуйста, введите шаблон запроса',
    advanced_config: 'Расширенные настройки',
    custom_params: 'Настройка параметров AI-модели',
    custom_params_key_label: 'Имя параметра (например: temperature)',
    custom_params_value_label: 'Значение параметра (например: 0.7)',
    proxy_placeholder: 'например: http://proxy.example.com:8080',
    timeout_placeholder: 'Тайм-аут (секунды)',
    timeout_required: 'Пожалуйста, введите тайм-аут',
    url_tip: `- **Описание**: API-адрес конечной точки AI-сервиса\n- **Пример**: \`https://api.deepseek.com/v1/chat/completions\``,
    api_key_tip: `- **Описание**: API-ключ, предоставленный поставщиком AI-сервиса\n- **Как получить**:\n  - OpenAI: Получить на сайте OpenAI\n  - DeepSeek: Зарегистрироваться и получить на сайте DeepSeek`,
    model_name_tip: `- **Описание**: Укажите имя используемой AI-модели\n- **Популярные модели**:\n  - \`gpt-3.5-turbo\` (OpenAI)\n  - \`gpt-4\` (OpenAI)\n  - \`deepseek-chat\` (DeepSeek)`,
    prompt_template_tip: `Шаблон запроса — это основа AI-анализа. Вы можете использовать {{$event}} для ссылки на каждое поле события. Подробную структуру события смотрите в [таблице истории оповещений](https://flashcat.cloud/docs/content/flashcat-monitor/nightingale-v9/usage/alert-notify/events/alert-history/). Начните с предоставленного шаблона по умолчанию.`,
    custom_params_tip: `- Настройка параметров AI-модели\n\nИспользуется для тонкой настройки поведения AI-модели:\n\n| Имя        | Описание           | Рекомендуется | Пример |\n| ---------- | ------------------ | ------------- | ------ |\n| temperature | Контролирует случайность | 0.3-0.7 | 0.7 |
| max_tokens  | Максимум токенов   | 200-500       | 300   |
| top_p       | Порог выборки      | 0.8-1.0       | 0.9   |
\n**Как настроить**:\n1. Нажмите кнопку + рядом с "Custom Params"\n2. Введите имя параметра (например: temperature)\n3. Введите значение параметра (например: 0.7)`,
    prompt_template_placeholder: `Пожалуйста, проанализируйте следующую информацию о тревожном событии и предоставьте краткое и ясное резюме на китайском языке:
Правило оповещения: {{$event.RuleName}}
Уровень серьезности: {{$event.Severity}}
Статус оповещения: {{if $event.IsRecovered}}Recovered{{else}}{{$event.Severity}} Triggered{{end}}
Время срабатывания: {{$event.TriggerTime}}
Значение триггера: {{$event.TriggerValue}}
Описание правила: {{$event.RuleNote}}
Теги: {{$event.Tags}}
Аннотации: {{$event.Annotations}}

Пожалуйста, предоставьте резюме объемом не более 100 слов, выделив ключевые моменты:
1. Какая система/сервис столкнулась с какой проблемой
2. Степень серьезности проблемы
3. Возможные последствия
4. Краткое предложение по устранению
Резюме должно быть кратким и помогать персоналу быстро понять ситуацию с оповещением.`,
  },
  script: {
    timeout: 'Время ожидания (в миллисекундах)',
    timeout_tooltip: 'Максимальное время выполнения скрипта, превышение которого приведет к его завершению',
    timeout_placeholder: 'Введите время ожидания',
    content: 'Содержимое скрипта',
    content_tooltip: 'Напишите скрипт для обработки событий, события будут переданы в скрипт в виде JSON-объекта, скрипт должен выводить JSON-объект в stdout',
    content_placeholder: 'Введите содержимое скрипта',
  },
  inhibit: {
    help: 'Процессор подавления событий. Когда отправляется одно предупреждение, он предотвращает уведомления о другом событии предупреждения, уменьшая количество уведомлений. Типичный сценарий: когда существует активная ошибка уровня P1 для того же правила предупреждения, игнорировать уведомления предупреждений уровня P2 и P3. Для получения дополнительной информации см. <a>документацию</a>',
    tip1: 'Когда <b>новое предупреждение</b> соответствует следующим условиям',
    tip2: 'и',
    tip3: 'секунд назад существуют <b>активные предупреждения</b>, соответствующие следующим условиям',
    tip4: 'и <b>новое предупреждение</b> и <b>активное предупреждение</b> имеют следующие общие элементы',
    tip5: 'Когда все вышеперечисленные условия выполнены, текущее предупреждение будет подавлено и больше не будет уведомлено',
    duration_required: 'Продолжительность подавления обязательна',
    duration_max: 'Продолжительность подавления не может превышать 600 секунд',
    match_label_keys: 'Метки',
    match_label_keys_required: 'Метки обязательны',
    match_attribute_keys: 'Атрибуты',
    match_attribute_keys_required: 'Атрибуты обязательны',
    keys_at_least_one_required: 'Требуется по крайней мере одна метка или атрибут',
    labels_conflict: 'Значения метки {{label}} различаются, подавление невозможно',
    attributes_conflict: 'Значения атрибута {{attribute}} различаются, подавление невозможно',
    preview:
      'Предварительный просмотр правила: Когда «<b>Новое предупреждение: {{newAlertLabelsAttrs}}</b>» и в течение последних «<b>{{duration}} секунд</b>» существует «<b>Активное предупреждение: {{activeAlertLabelsAttrs}}</b>», и оба совпадают в «<b>{{matchLabelsAttrs}}</b>», подавить уведомления нового предупреждения.',
    labels_filter: {
      label: 'Метки',
      label_tip:
        'Подавлять только события предупреждений, которые соответствуют этим условиям соответствия меток, используется для сужения области воздействия. Не сконфигурировано означает отсутствие ограничений. Поддерживает выпадающий выбор существующих ключей меток (рекомендуется) или ручной ввод',
      label_placeholder: 'Введите или выберите ключи меток для сопоставления, такие как app / cluster / alertname',
    },
    labels_filter_value_placeholder: 'Введите вручную или выберите значения меток для сопоставления',
    attributes_filter: {
      label: 'Атрибуты',
      label_tip:
        'Ограничить область подавления по атрибутам событий: будут подавлены только предупреждения, одновременно соответствующие этим атрибутам; оставьте пустым для применения ко всем предупреждениям',
    },
    active_event_labels_filter: {
      label: 'Метки',
      label_tip: `**Используется для ограничения области активных предупреждений**
- Не настроено: означает, что фильтрация по меткам не используется
- Настроено: можно выбрать существующие ключи меток из выпадающего списка (рекомендуется) или ввести ключи меток вручную. Только когда активные предупреждения одновременно соответствуют этим условиям меток, они попадут в область фильтрации.

Пример: заполните service=mon, что означает, что только когда событие содержит метку service=mon, оно будет участвовать в последующей логике подавления.`,
    },
    active_event_attributes_filter: {
      label: 'Атрибуты',
      label_tip: `**Используется для ограничения области активных предупреждений**
- Не настроено: означает, что фильтрация по атрибутам не используется
- Настроено: только когда активные предупреждения одновременно соответствуют этим условиям атрибутов, они будут отфильтрованы.

Пример: заполните Бизнес-группа==DefaultBusiGroup, что означает, что только когда атрибут "Бизнес-группа" активного события равен DefaultBusiGroup, он будет отфильтрован для последующего процесса подавления событий`,
    },
  },
  inhibit_qd: {
    help: 'Подавление событий на основе результатов запроса: когда срабатывает предупреждение, выполняется следующий запрос данных; если возвращается по крайней мере одна запись, текущее предупреждение подавляется (не уведомляется); если данных нет, уведомление происходит как обычно. Дополнительные объяснения см. в <a>документации</a>',
    t_1: 'и запросить следующие <b>данные</b>',
  },
  annotation_qd: {
    help: 'Процессор дополнительных запросов — это способ улучшения оповещений. При срабатывании оповещения он может запрашивать связанную информацию из источников данных, таких как журналы, и прикреплять их к оповещению. Подробности см. в <a>документации</a>',
    query_configs: 'Запрос данных',
    use_event_datasource: 'Использовать источник данных события оповещения',
    use_event_datasource_help: 'При включении можно выбрать только образцы событий оповещений, соответствующие типу источника данных',
    datasource_cate_required: 'Тип источника данных обязателен',
    datasource_ids_required: 'Источник данных обязателен',
    select_alert_event_btn: 'Выбрать образец события оповещения',
    select_alert_event_tip: 'Выберите образец события оповещения для отображения переменных в операторе запроса и предварительного просмотра данных',
    select_alert_event_label: 'Выбранный образец события оповещения',
    query_required: 'Условие запроса обязательно',
    sql_limit_valid: 'SQL-запрос должен содержать предложение LIMIT',
    oracle_sql_limit_valid: 'SQL-запрос должен содержать предложение ROWNUM',
    annotation_configs: 'Добавление данных',
    annotation_configs_tip: 'Настройте Key/Value для добавления результатов запроса в информацию об оповещении',
    annotation_key_tip: 'Определите ключ нового поля. Рекомендуется использовать английские буквы',
    annotation_val_tip: 'Шаблон значения нового поля, см. синтаксис в документации',
    annotation_key_placeholder: 'Имя дополнительного поля',
    annotation_val_placeholder: 'Содержимое дополнительного поля, поддерживает синтаксис шаблонов для заполнения результатов запроса в виде переменных',
    annotation_key_required: 'Имя дополнительного поля обязательно',
    annotation_val_required: 'Содержимое дополнительного поля обязательно',
    data_preview: 'Предварительный просмотр данных',
    data_preview_query: 'Оператор запроса',
    data_preview_no_eventid: 'Пожалуйста, сначала выберите событие оповещения',
    query_limit: 'Ограничение количества возвращаемых записей',
  },
  event_recover: {
    help: 'Процессор самовосстановления событий оповещения. Используется для выполнения оболочечных скриптов на машинах при срабатывании оповещения. Может использоваться для получения соответствующей информации об оповещении или выполнения задач самовосстановления。 <a>документация</a>',
    title: 'Самовосстановление оповещения',
    create_btn: 'Создать шаблон самовосстановления',
    tpl_id: 'Шаблон самовосстановления',
    tpl_id_required: 'Шаблон самовосстановления обязателен',
    host: 'Целевая машина',
    host_placeholder: 'По умолчанию может быть оставлена пустой. Если пусто, машина для выполнения будет получена из метки ident в событии',
    args: 'Параметры',
    args_tip: 'Параметры, добавляемые после скрипта. Несколько параметров разделяются двойной запятой,,. Например: arg1,,arg2,,arg3',
    save_result: 'Сохранить результат выполнения',
    save_result_tip: 'Сохраните результат выполнения скрипта в событие оповещения',
    timeout: 'Время ожидания выполнения',
    timeout_tip: 'Если скрипт не может быть выполнен в течение времени ожидания, результат не будет получен',
    timeout_max_warning: 'Время ожидания выполнения не может превышать 60 секунд',
    select_host: 'Выбрать целевую машину',
  },
  alert_shot: {
    help: 'Помощь',
    title: 'Скриншот оповещения',
    shot_type: {
      label: 'Тип объекта',
      options: {
        board: 'Панель управления',
        url: 'URL адрес',
      },
    },
    advanced_settings: 'Расширенные настройки',
    board_shot_opts: {
      busi_group: 'Бизнес-группа',
      board_id: 'Панель управления',
      board_url: 'URL панели управления',
      timeout: 'Время ожидания (мс)',
      width: 'Ширина изображения',
    },
    url_shot_opts: {
      url: 'URL адрес',
      headers: 'Заголовки запроса',
      proxy: 'Настройки прокси',
      insecure_skip_verify: 'Пропустить проверку сертификата',
      timeout: 'Время ожидания (мс)',
      width: 'Ширина изображения',
    },
  },
};

export default ru_RU;
