const zh_CN = {
  title: '日志检索',
  tab: {
    rename: '重命名',
  },
  query: '查询条件',
  query_is_required: '查询条件不能为空',
  execute: '查询',
  mode: {
    label: '模式',
    raw_logs: '日志原文',
    statistical_charts: '统计图表',
  },
  mode_switch: {
    confirm_title: '切换模式确认',
    confirm_content: '当前统计图表模式的查询条件包含管道符（|），该语法在日志原文模式下不可用。切换后将清空当前查询条件，是否继续？',
    confirm_ok: '继续切换',
    confirm_cancel: '取消',
  },
  before_query: '请点击 <b>查询</b>，展示数据',
  loading: '数据加载中 ...',
  no_data: '查询无数据',
  histogram_hide: '隐藏图表',
  histogram_show: '显示图表',
  share_btn: '分享链接',
  share_tip: '点击复制分享链接',
  log_viewer_drawer_trigger_tip: '点击查看日志详情',
  log_viewer_drawer_title: '日志详情',
  copy_to_clipboard: '复制到剪贴板',
  unindexable: '字段未开启统计，无法进行统计分析',
  topn_no_data: '无数据',
  stats: {
    unique_count: '唯一值个数',
    min: '最小值',
    max: '最大值',
    sum: '求和',
    avg: '平均值',
    exist_ratio: '字段存在的日志比例',
    median: '中位数',
    p95: '百分位数(P95)',
  },
  field_popover_info_alert: '点击数值可查看统计图表和 SQL',
  field_search_placeholder: '搜索字段',
  field_list: {
    show_fields: '显示字段',
    available_fields: '可用字段',
  },
  field_actions: {
    and: '添加到本次检索',
    not: '从本次检索中排除',
    exists: '过滤存在该字段的文档',
  },
  field_values_topn: {
    title: '前 {{n}} 个值',
    settings: {
      title: '前 N 个值设置',
    },
    no_data: '该字段存在于 mapping 中，但不存在于显示的 500 个文档中',
    quick_view_count: '日志数',
    quick_view_ratio: '占比',
  },
  empty_value_not_supported_tip: '暂不支持对空值做检索',
  unsupported_datasource_type: '无法渲染不支持的数据源类型：{{type}}',
  no_supported_datasource_types_title: '没有可用的数据源类型',
  no_supported_datasource_types_desc: '可前往<a>数据源管理</a>页面配置或联系管理员配置，目前支持的数据源类型（{{types}}），',
  field_tip: '点击查看统计信息',
  field_value_statistic: {
    view_statistic: '查看统计值',
    view_timeseries: '查看时序图',
  },
  field_type: '类型',
  field_type_map: {
    float: '浮点数',
    float64: '64位浮点数',
    scaled_float: '缩放浮点数',
    double: '双精度浮点数',
    integer: '整数',
    int64: '64位整数',
    long: '长整数',
    date: '日期',
    date_nanos: '纳秒日期',
    string: '字符串',
    text: '字符串',
    nested: '嵌套对象',
    histogram: '直方图型',
    boolean: '布尔值',
  },
  logs: {
    title: '日志数据',
    stream_fields_count: '{{count}} 个',
    text: '日志文本',
    duration: '耗时',
    count: '条数',
    filter_fields: '筛选字段',
    settings: {
      mode: {
        origin: '原始',
        table: '表格',
        timeseries: '时序图',
        clustering: '聚类',
      },
      breakLine: '换行',
      reverse: '时间',
      lines: '行号',
      time: '日志时间',
      organizeFields: {
        title: '字段列设置',
        allFields: '可用字段',
        showFields: '显示字段',
        showFields_empty: '日志默认显示全部字段',
        tip: '当前只显示字段 {{fields}}，可点击设置图标设置显示所有字段',
      },
      jsonSettings: {
        title: 'JSON 设置',
        displayMode: '默认展示类型',
        displayMode_tree: '树形展示',
        displayMode_string: '字符串展示',
        expandLevel: '默认展开层级',
      },
      pageLoadMode: {
        title: '翻页模式',
        pagination: '分页器',
        infiniteScroll: '滚动加载更多',
      },
      topNSettings: {
        title: '前 N 个值设置',
      },
    },
    fieldLabelTip: '字段未开启统计，无法进行统计分析',
    filterAnd: '添加 "{{token}}" 到本次检索',
    filterNot: '从本次检索中排除 "{{token}}"',
    filterAllAnd: '添加全部到本次检索',
    filterAllNot: '从本次检索中排除全部',
    filterExists: '过滤存在该字段的文档',
    add_drilldown_link: '添加下钻链接',
    drilldown_link_default_name: '下钻链接',
    total: '日志条数',
    stack_group_by_tip: '以此字段值展示堆叠趋势图',
    collapse: '收起',
    expand: '展开',
    copy_field_value: '拷贝字段值',
  },
  clustering: {
    count: '数量',
    log_data: '日志数据',
    row_number: '行号',
    log_statistics: '日志统计',
    back_to_all_logs: '返回所有日志',
    all_log_statistics: '所有日志统计',
    current_page_field: '目前对当前页的字段',
    aggregate: '聚类，',
    cannot_aggregate: '暂时无法对',
    full_aggregate_logs: '日志全量聚类',
    need_aggregate: '若需对全部',
    click_to_aggregate: '条日志聚类，请点击',
    full_aggregate: '全量聚类',
    field_label: '聚类字段',
    scope_current_page: '当前页面',
    scope_current_page_desc: '仅对当前页面的字段进行聚类',
    scope_full: '全量聚类',
    scope_full_desc_prefix: '对当前查询出的',
    scope_full_desc_disable_prefix: '暂时无法对查询出的',
    scope_full_desc_suffix: '条日志进行聚类',
    scope_label: '范围',
    aggregate_field: '聚类字段:',
    log_count: '日志量:',
    duration: '耗时:',
    top5_title: '前 5 个值',
    no_data: '暂无数据',
    loading_title: '聚类分析中，请稍等',
    loading_info: '聚类日志量：',
    loading_field: '聚类字段：',
    loading_tip: '请勿关闭此页。需要新的查询，',
    loading_new_tab: '打开新的标签页',
    loading_tip_suffix: '进行日志检索',
    sampled_tip: '日志数量过多，当前聚类结果基于采样日志生成',
  },
  view_placeholder: '日志视图',
};
export default zh_CN;
