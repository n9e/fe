const ko_KR = {
  ...
  notification_configuration: {
    ...
    test_with_recovery: '복구도 함께 테스트(이슈에 댓글 후 종료)',
    user_info: {
      user_ids: '수신자',
      user_group_ids: '수신 팀',
    ...
    jira: {
      project: '프로젝트',
      project_tip: '이슈가 생성될 프로젝트입니다. 목록은 알림 매체의 자격 증명으로 실시간 조회하며, 불러오지 못하면 프로젝트 키를 직접 입력할 수 있습니다.',
      project_empty: '프로젝트 없음: 매체에 설정된 계정에 Browse projects 권한이 없습니다',
      issue_type: '이슈 유형',
      issue_type_tip: '프로젝트에 있는 기존 이슈 유형 중에서 고르면 되며 전용 유형을 만들 필요는 없습니다. Alert 유형이 있으면 기본으로 선택됩니다.',
      missing_permissions: '계정에 이 프로젝트의 다음 권한이 없습니다: {{list}}',
      required_fields: '이 이슈 유형의 필수 필드가 「사용자 지정 필드」에 입력되지 않았습니다: {{list}}. 이슈 생성이 실패합니다',
      on_resolve: '복구 시',
      on_resolve_tip: '이슈 종료: 댓글을 남기고 완료 상태로 자동 전환합니다(전환 이름 불필요). 댓글만: 이슈는 열린 상태로 둡니다. 처리 안 함: 이슈를 건드리지 않습니다.',
      on_resolve_close: '이슈 종료',
      on_resolve_comment: '댓글만',
      on_resolve_none: '처리 안 함',
      advanced: '고급',
      resolve_transition: '종료 전환',
      resolve_transition_tip: '워크플로에 완료로 가는 전환이 여러 개이고 특정 전환을 쓰려는 경우에만 입력합니다. 대소문자를 구분하지 않으며 숫자 ID도 됩니다.',
      resolve_transition_placeholder: '비워 두면 자동 선택',
      on_repeat: '반복 알림 시',
      ...
      priority_fill_common: '일반 매핑 채우기',
      labels: '고정 레이블',
      labels_tip: '모든 이슈에 붙는 레이블입니다(공백은 _로 바뀝니다). 중복 제거를 위해 eventHash=<이벤트 해시> 레이블이 항상 붙으니 Jira에서 삭제하지 마세요.',
      labels_placeholder: 'Enter로 추가',
      tags_as_labels: '알림 태그를 레이블로',
      tags_as_labels_tip: '알림의 각 태그를 Jira 레이블 key=value로 붙입니다(공백은 _로 대체, 최대 20개).',
      fields: '사용자 지정 필드',
      fields_tip: '이슈 유형의 필수 사용자 지정 필드를 여기서 채웁니다. 값이 올바른 JSON이면 JSON으로(선택 목록이면 {"value":"prod"} 등), 아니면 텍스트로 보냅니다.',
    ...
    discord: {
      ...
      webhook_url_history_placeholder: 'Webhook URL을 붙여 넣거나 클릭해 이전에 입력한 것을 선택하세요',
      bot_name: '이름',
    ...
    jsm_alert: {
      api_key: 'API 키',
      api_key_tip:
        'JSM에서 팀 → Integrations → Add integration → API를 열고 API 키를 복사합니다. 키가 알림을 받을 팀을 결정하며 알림 기록에는 마스킹되어 표시됩니다. {{.변수명}}을 지원합니다.',
  ...
  escalations: {
    ...
    labels_filter: {
      label_tip:
        '이 레이블 조건에 맞는 알림 이벤트만 에스컬레이션해 영향 범위를 좁힙니다. 비워 두면 제한하지 않습니다. 기존 레이블 key를 목록에서 고르는 방법을 권하며 직접 입력할 수도 있습니다',
    },
    attributes_filter: {
  ...
  notify_aggr_configs: {
    ...
    labels_filter: {
      label_tip:
        '이 레이블 조건에 맞는 알림 이벤트만 묶어 보내 영향 범위를 좁힙니다. 비워 두면 제한하지 않습니다. 기존 레이블 key를 목록에서 고르는 방법을 권하며 직접 입력할 수도 있습니다',
    },
    attributes_filter: {
      label_tip:
        '이 레이블 필터에 맞는 알림만 집계에 참여하며 맞지 않는 알림은 이 규칙의 영향을 받지 않습니다<br />조건 사이는 그리고로 이어지며 아래의 속성 필터와도 그리고로 이어집니다',
    },
    label_keys: {
  ...
  statistics: {
    ...
    escalation_events_tip:
      '에스컬레이션 규칙에 걸려 우선순위가 올라간 이벤트 수입니다. 이 수가 많다면 대개 처리 시간이 길다는 뜻이므로 <b>대응 SLA, 에스컬레이션 임계값, 알림 억제 전략</b>을 손볼 필요가 있습니다',
