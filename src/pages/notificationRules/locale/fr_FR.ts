const fr_FR = {
  ...
  notification_configuration: {
    ...
    severities_tip:
      "Choisissez les niveaux de gravité à notifier ; seuls les niveaux cochés le sont. Si aucun des trois n'est coché, le média ne correspondra à aucun événement, ce qui revient à le désactiver",
    ...
    label_keys_tip:
      "Une règle de notification peut ne concerner que les événements retenus par un filtre sur les étiquettes ; sans configuration, aucune restriction ne s'applique",
    ...
    test_with_recovery: 'Tester aussi la résolution (commenter et fermer le ticket)',
    user_info: {
    ...
    jira: {
      project: 'Projet',
      project_tip:
        'Le projet dans lequel les tickets sont créés. La liste est chargée en direct avec les identifiants du média ; si elle ne se charge pas, saisissez la clé du projet.',
      project_empty: "Aucun projet : le compte du média n'a pas l'autorisation Browse projects",
      issue_type: 'Type de ticket',
      issue_type_tip: "Choisissez un type de ticket existant du projet ; inutile d'en créer un dédié. Si le projet a un type Alert, il est sélectionné par défaut.",
      missing_permissions: "Le compte n'a pas ces autorisations dans le projet : {{list}}",
      required_fields: 'Des champs obligatoires de ce type de ticket ne sont pas renseignés dans Champs personnalisés : {{list}} ; la création échouera',
      on_resolve: 'À la résolution',
      on_resolve_tip:
        "Fermer le ticket : commenter et passer automatiquement le ticket à un statut Terminé (sans nom de transition). Commenter uniquement : le ticket reste ouvert. Ne rien faire : le ticket n'est pas modifié.",
      on_resolve_close: 'Fermer le ticket',
      on_resolve_comment: 'Commenter uniquement',
      on_resolve_none: 'Ne rien faire',
      advanced: 'Avancé',
      resolve_transition: 'Transition de fermeture',
      resolve_transition_tip:
        'Utile seulement si le workflow a plusieurs transitions vers Terminé et que vous en voulez une précise. Insensible à la casse ; un ID numérique fonctionne aussi.',
      resolve_transition_placeholder: 'Vide = choix automatique',
      on_repeat: 'Lors des notifications répétées',
      on_repeat_tip:
      ...
      priority_unset: 'Non défini',
      priority_fill_common: 'Remplir la correspondance usuelle',
      labels: 'Étiquettes fixes',
      labels_tip:
        "Étiquettes ajoutées à chaque ticket (les espaces deviennent _). Une étiquette eventHash=<hash de l'événement> est toujours ajoutée pour la déduplication ; ne la supprimez pas dans Jira.",
      labels_placeholder: 'Appuyez sur Entrée pour ajouter',
      tags_as_labels: "Tags d'alerte en étiquettes",
      tags_as_labels_tip: "Chaque tag d'alerte devient une étiquette Jira key=value (espaces remplacés par _, 20 au maximum).",
      fields: 'Champs personnalisés',
      fields_tip:
        'Renseignez ici les champs personnalisés obligatoires du type de ticket. Une valeur JSON valide est envoyée en JSON (par exemple {"value":"prod"} pour une liste), sinon en texte.',
  ...
  escalations: {
    ...
    title_tip:
      "Passé le délai fixé sans résolution, le système remonte la notification vers le canal désigné selon les conditions ci-dessous, pour éviter qu'elle reste longtemps sans suite. Voir la <a>documentation</a>",
    ...
    labels_filter: {
      label_tip:
        "Seuls les événements remplissant ces conditions d'étiquette sont remontés, ce qui restreint la portée ; sans configuration, aucune restriction ne s'applique. Choisissez de préférence une clé existante dans la liste, ou saisissez-la",
    },
    attributes_filter: {
  ...
  notify_aggr_configs: {
    ...
    labels_filter: {
      label_tip:
        "Seuls les événements remplissant ces conditions d'étiquette sont regroupés, ce qui restreint la portée ; sans configuration, aucune restriction ne s'applique. Choisissez de préférence une clé existante dans la liste, ou saisissez-la",
    },
    attributes_filter: {
      label_tip:
        "Seules les alertes correspondant à ce filtre d'étiquettes entrent dans l'agrégation ; les autres ne sont pas touchées par cette règle<br />Les conditions se combinent par ET, y compris avec le filtre d'attributs ci-dessous",
    },
    label_keys: {
  ...
  statistics: {
    ...
    escalation_events_tip:
      "Nombre d'événements dont la priorité a été relevée par une règle d'escalade. Un chiffre élevé traduit en général des délais de traitement longs et invite à revoir <b>le SLA de réponse, les seuils d'escalade ou la stratégie de réduction des alertes</b>",
    noise_reduction_ratio: 'Taux de réduction du bruit sur {{days}} jours',
    noise_reduction_ratio_tip:
      "Taux de réduction du bruit = <b>(1 − notifications réellement envoyées ÷ événements d'alerte d'origine) × 100 %</b>. Plus il approche de <b>100 %</b>, meilleure est la <b>réduction du bruit</b>",
  },
  tabs: {
