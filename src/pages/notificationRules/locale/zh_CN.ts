const zh_CN = {
  ...
  notification_configuration: {
    ...
    jira: {
      ...
      reopen_transition: '重开动作',
      reopen_transition_tip: '工单关闭后同一告警又触发：在时间窗内则执行这个动作重开原工单，超出时间窗则新建工单。留空则总是新建。',
      reopen_duration: '重开时间窗（分钟）',
      reopen_duration_tip: '默认 1440 分钟（24 小时）。',
      wont_fix_resolution: '忽略的解决结果',
      wont_fix_resolution_tip: '工单以这个解决结果关闭时不重开，直接新建。',
    ...
    jsm_history: {
      label: "复用填过的 key",
      tip: "本人所在团队的规则里这个媒介填过的 key，选一条会把整组参数填回来",
      placeholder: "选择之前填过的 key",
    },
    jsm_alert: {
      api_key: "API Key",
