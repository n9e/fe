const zh_CN = {
  title: '通知规则',
  empty_guide: {
    title: '还没有通知规则',
    desc: '通知规则决定"告警发给谁、用哪个媒介"。配置后，告警事件才会真正推送到钉钉 / 邮件等渠道。',
    config_channel: '先去配置通知媒介',
  },
  rule_select: {
    label: '通知规则',
    select: '选择通知规则',
    create: '新建通知规则',
    view: '查看',
    manage: '通知规则管理',
    total: '共 {{total}} 条',
    footer_total: '共 {{total}} 条规则',
    quick_create: {
      action: '快捷创建',
      title: '快捷创建通知规则',
      hint: '粘贴 IM 机器人 Webhook URL 或 Flashduty 集成地址（含 integration_key），将自动识别钉钉 / 企微 / 飞书卡片 / Lark 卡片 / Flashduty，查重后复用或创建通知规则。',
      url_label: 'Webhook / 集成地址',
      url_placeholder: '例如：https://oapi.dingtalk.com/robot/send?access_token=xxx\n或：https://api.flashcat.cloud/event/push/alert/n9e?integration_key=xxx',
      url_required: '请粘贴 Webhook URL 或 Flashduty 集成地址',
      name_label: '通知规则名称',
      name_placeholder: '粘贴 Webhook 后自动生成，可修改',
      name_required: '请填写通知规则名称',
      user_group_required: '请选择授权团队',
      user_group_placeholder: '请选择授权团队',
      detected: '识别为：{{channel}}（尾号 {{suffix}}）',
      invalid_url: 'URL 格式不正确',
      missing_param: 'URL 缺少 {{key}}',
      unrecognized: '无法识别类型（支持钉钉 / 企微 / 飞书卡片 / Lark 卡片 / Flashduty）',
      reused_rule: '已存在相同 Token 的通知规则，已自动选中',
      created: '通知规则已创建并选中',
      create_channel_no_perm: '缺少 {{channel}} 通知媒介，且当前用户无通知媒介创建权限，请联系管理员先创建',
      create_channel_failed: '创建通知媒介失败：{{channel}}',
      create_rule_failed: '创建通知规则失败',
      channel_description: '由「快捷创建」自动创建',
      rule_description: '由「快捷创建」根据 Webhook URL 创建',
      submit: '创建',
    },
  },
  basic_configuration: '基础配置',
  basic_configuration_desc: '通知规则的名称、授权团队与备注',
  name_auto_tip: '选择通知媒介与接收团队后会自动生成名称，可随时修改',
  name_auto_separator: '、',
  add_note_btn: '添加备注',
  user_group_ids: '授权团队',
  user_group_ids_tip: '这里填写的团队中的用户，将有权限管理或查看此通知规则',
  enabled_tip: '是否启用此通知规则',
  note_tip: '可在此补充该通知规则的详细信息或说明，便于日后维护',
  notification_configuration: {
    test_with_recovery_jsm: '同时测试恢复（关闭告警）',
    title: '通知配置',
    section_desc: '决定告警发给谁、用什么媒介发：选择通知媒介、消息模板与接收对象，可添加多条',
    item_title: '通知配置',
    add_btn: '添加通知配置',
    filters: {
      title: '筛选条件',
      tip: '限制本条通知配置只对符合条件的告警事件生效：适用级别、时段、标签、属性。不配置表示不做限制',
      severities_all: '全部级别',
      severities_none: '未勾选级别，不会匹配任何事件',
      time_ranges_count: '{{count}} 个时段',
      label_keys_count: '{{count}} 个标签条件',
      attributes_count: '{{count}} 个属性条件',
      no_extra: '时段/标签/属性不限',
    },
    test_mode: {
      history: '选择历史事件',
      mock: '使用模拟事件',
    },
    mock_test: {
      empty_alert: '当前环境还没有告警事件，可使用模拟事件直接测试通知渠道',
      switch_btn: '去使用模拟事件',
      desc: '将向该通知配置的媒介与接收对象发送一条内置模拟告警事件，用于验证通知渠道是否可用；模拟测试不校验筛选条件',
      preview_title: '模拟事件预览',
      preview_rule_name: '规则名称',
      preview_severity: '告警级别',
      preview_tags: '标签',
      rule_name: '通知测试模拟事件',
    },
    channel: '通知媒介',
    channel_tip: '选择使用哪种媒介发送告警事件通知，如果已有媒介不满足需求，可以联系管理员创建新的媒介',
    channel_msg: '请选择通知媒介',
    template: '消息模板',
    template_tip: '通知内容的模板，可以根据不同的场景使用不同的模板',
    template_msg: '请选择消息模板',
    severities: '适用级别',
    severities_tip: '选择要对哪个等级的告警事件进行通知，只有勾选上的级别，才会被通知。如果三个等级都没有勾选，这个媒介就匹配不到告警事件了，相当于禁用了这个媒介',
    time_ranges: '适用时段',
    time_ranges_tip: '通知规则可以限制仅在部分时段生效，不配置表示不做限制',
    effective_time_start: '开始时间',
    effective_time_end: '结束时间',
    effective_time_week_msg: '请选择生效星期',
    effective_time_start_msg: '请选择开始时间',
    effective_time_end_msg: '请选择结束时间',
    fetch_integration_key_failed_remove: '获取以下 PagerDuty key 失败：{list}，请尝试重新点击选取',
    label_keys: '适用标签',
    label_keys_tip: '通知规则可以限制仅对符合条件（通过事件标签做筛选）的部分告警事件生效，不配置表示不做限制',
    attributes: '适用属性',
    attributes_value: '属性值',
    attributes_tip: '通知规则可以限制仅对符合某些事件属性的部分告警事件生效，不配置表示不做限制',
    attributes_options: {
      group_name: '业务组',
      cluster: '数据源',
      is_recovered: '是恢复事件？',
      rule_id: '告警规则',
      severity: '告警等级',
      target_group: '机器业务组',
    },
    run_test_btn: '通知测试',
    run_test_btn_tip: '可以选择几个已经产生的事件，测试一下此通知配置是否正确，如果正确，应该会收到相关的通知消息',
    run_test_request_result: '测试通知已发送，通知目标返回响应如下：',
    test_with_recovery: '同时测试恢复（评论并关闭工单）',
    user_info: {
      user_ids: '接收人',
      user_group_ids: '接收团队',
      error: '接收人和接收团队不能同时为空',
    },
    flashduty: {
      ids: '协作空间',
    },
    jira: {
      project: '项目',
      project_tip: '工单创建到哪个项目。列表用媒介里的凭证实时拉取，拉取失败时可以直接输入项目 key。',
      project_empty: '没有可选项目：媒介里配置的账号没有 Browse projects 权限',
      issue_type: '工作类型',
      issue_type_tip: '从项目已有的工作类型里选，无需专门新建。项目里有 Alert 类型时默认选中它。',
      missing_permissions: '账号在该项目缺少权限：{{list}}',
      required_fields: '该工作类型有必填字段没在「自定义字段」里填写：{{list}}，建单会失败',
      on_resolve: '恢复时',
      on_resolve_tip: '关闭工单：评论并自动流转到「完成」状态，无需填写动作名；只评论：工单保持打开；不处理：不动工单。',
      on_resolve_close: '关闭工单',
      on_resolve_comment: '只评论',
      on_resolve_none: '不处理',
      advanced: '高级',
      resolve_transition: '关闭动作名称',
      resolve_transition_tip: '工作流里有多个能到「完成」的动作、想指定其中一个时填，不区分大小写，也可以填数字 ID。',
      resolve_transition_placeholder: '留空自动选择',
      on_repeat: '重复通知时',
      on_repeat_tip: '告警持续期间，重复通知不会再建新单。选「追加评论」则每次追加一条评论（关注者会收到 Jira 通知）。',
      on_repeat_none: '不处理',
      on_repeat_comment: '追加评论',
      priority_map: '级别与优先级映射',
      priority_map_tip: '把告警级别对应到 Jira 的优先级。留空则不设置优先级；工作类型的界面上没有优先级字段时必须留空，否则建单失败。',
      priority_unset: '不设置',
      priority_fill_common: '填入常用映射',
      labels: '固定标签',
      labels_tip: '每张工单都带上的标签（空格会替换成 _）。系统还会自动加一个 eventHash=<事件哈希> 标签用于去重，请不要在 Jira 里删除它。',
      labels_placeholder: '回车添加',
      tags_as_labels: '告警标签写入 Jira 标签',
      tags_as_labels_tip: '把每个告警标签写成一个 Jira 标签 key=value（空格替换成 _，最多 20 个）。',
      fields: '自定义字段',
      fields_tip: '工作类型有必填的自定义字段时在这里补。值是合法 JSON 时按 JSON 提交（如下拉字段填 {"value":"生产"}），否则按文本提交。',
      field_value_placeholder: '值，如 {"value":"生产"}',
      field_add: '添加字段',
    },
    discord: {
      webhook_url: 'Webhook 地址',
      webhook_url_tip:
        '在 Discord 里打开 Server Settings → Integrations → Webhooks → New Webhook，选择频道后点 Copy Webhook URL。地址本身就是凭证，通知记录里会掩码显示。支持用 {{.变量名}} 引用变量配置。',
      webhook_url_invalid: '应形如 https://discord.com/api/webhooks/<id>/<token>',
      webhook_url_history_placeholder: '粘贴 Webhook 地址，或点击选择之前填过的',
      bot_name: '名称',
      bot_name_tip: '给这个 Webhook 起个好认的名字：通知记录里的目标显示它，其他规则也可以按名称直接复用这个地址',
      target: '发送到',
      target_tip:
        '按 Webhook 绑定的频道类型选：\n· 频道：发到 Webhook 绑定的文字频道，最常用\n· 新建论坛帖子：Webhook 绑定的是论坛频道时用，每条通知（包括恢复）都新开一个帖子\n· 已有线程或论坛帖子：发到 Webhook 所在频道里某个已有的线程或论坛帖子，所有通知集中在一处，需要填线程 ID',
      target_channel: '频道',
      target_forum_post: '新建论坛帖子',
      target_thread: '已有线程或论坛帖子',
      thread_name: '帖子标题',
      thread_name_tip: '每次通知都会新建一个帖子，标题支持模板变量（如 {{$event.RuleName}}，最多 100 字符）。要发到同一个已有帖子，请选「已有线程或论坛帖子」',
      thread_id: '线程 ID',
      thread_id_tip: '在 User Settings → Advanced 打开 Developer Mode，然后右键线程或帖子 → Copy ID',
      thread_id_invalid: '必须是纯数字 ID',
    },
    jsm_alert: {
      api_key: 'API Key',
      api_key_tip: '在 JSM 里打开团队 → Integrations → Add integration → API，复制 API Key。key 决定告警归哪个团队，通知记录里会掩码显示。支持用 {{.变量名}} 引用变量配置。',
      api_key_history_placeholder: '粘贴 API Key，或点击选择之前填过的',
      bot_name: '名称',
      bot_name_tip: '给这个 key 起个好认的名字（如团队名）：通知记录里的目标显示它，其他规则也可以按名称直接复用这个 key',
    },
    slackwebhook: {
      webhook_url: 'Webhook 地址',
      webhook_url_tip:
        '在 https://api.slack.com/apps 打开你的应用 → Incoming Webhooks → 打开 Activate Incoming Webhooks → Add New Webhook，选择频道后复制生成的地址。地址本身就是凭证，通知记录里会掩码显示。支持用 {{.变量名}} 引用变量配置。',
      webhook_url_invalid: '应形如 https://hooks.slack.com/services/T.../B.../...',
      webhook_url_history_placeholder: '粘贴 Webhook 地址，或点击选择之前填过的',
      bot_name: '名称',
      bot_name_tip: '给这个 Webhook 起个好认的名字（建议用频道名）：通知记录里的目标显示它，其他规则也可以按名称直接复用这个地址',
    },
    mattermostwebhook: {
      webhook_url: 'Webhook 地址',
      webhook_url_tip:
        '在 Mattermost 左上角菜单 → Integrations → Incoming Webhooks → Add Incoming Webhook，选择频道并保存后复制生成的地址。地址本身就是凭证，通知记录里会掩码显示。支持用 {{.变量名}} 引用变量配置。',
      webhook_url_invalid: '应形如 https://mattermost.example.com/hooks/<id>',
      webhook_url_history_placeholder: '粘贴 Webhook 地址，或点击选择之前填过的',
      bot_name: '名称',
      bot_name_tip: '给这个 Webhook 起个好认的名字（建议用频道名）：通知记录里的目标显示它，其他规则也可以按名称直接复用这个地址',
    },
    pagerduty: {
      services: '服务/集成',
    },
  },
  user_group_id_invalid_tip: '授权团队不存在',
  channel_invalid_tip: '通知媒介不存在',
  disabled: '停用',
  pipeline_configuration: {
    title: '事件处理工作流',
    section_desc: '通知发送前，先经事件处理工作流对告警事件做加工（如打标、富化、降噪）',
    manage_btn: '管理事件处理工作流',
    name_placeholder: '请选择事件处理工作流',
    name_required: '事件处理工作流不能为空',
    add_btn: '添加事件处理工作流',
    disable: '禁用',
    enable: '启用',
  },
  escalations: {
    title: '升级配置',
    section_desc: '告警长时间未恢复或未认领时，将通知升级到指定渠道，避免无人跟进',
    title_tip: '当告警超过设定时长且未恢复时，系统将按下方条件把通知升级到指定渠道，避免长期无人跟进，详细配置参考 <a>使用文档</a>',
    item_title: '通知升级',
    item_add_btn: '添加通知升级',
    interval: '检测周期',
    interval_required: '检测周期不能为空',
    duration_required: '持续时间不能为空',
    duration_1: '异常事件已超过',
    duration_2: '，仍处于',
    duration_3: '状态时，使用本配置发送通知。',
    repeating_notification: '循环通知设置',
    repeating_notification_tip: '如果此开关关闭，相同事件的告警，升级通知只会通知 1 次',
    repeating_notification_1: '每隔',
    repeating_notification_2: '分钟通知 1 次，最多循环通知',
    repeating_notification_3: '次',
    notification_interval_required: '通知间隔不能为空',
    notification_max_times_required: '最多循环通知次数不能为空',
    event_status_options: {
      0: '未恢复',
      1: '未恢复且未认领',
    },
    time_ranges: {
      label_tip: '可以限制仅在勾选的星期与时间段内触发升级，不配置表示不做限制',
    },
    labels_filter: {
      label_tip: '仅对满足这些标签条件的告警事件执行升级通知。用于缩小影响范围，不配置表示不做限制。支持下拉选择已有标签键（推荐），也可手动输入',
    },
    attributes_filter: {
      label_tip: '仅对同时匹配这些属性的告警启用升级；不配置表示不做限制。多个条件为且关系',
    },
  },
  notify_aggr_configs: {
    title: '聚合配置',
    section_desc: '将同类告警按标签/属性维度聚合为一条通知，减少打扰',
    enable: '开启聚合',
    group_enable: '细粒度聚合',
    group_title: '细粒度聚合',
    group_add_btn: '添加细粒度聚合',
    group_tip1: '满足以下条件',
    group_tip2: '按照以下维度聚合到一组进行通知',
    group_label_keys: '标签',
    group_label_keys_required: '标签不能为空',
    group_attribute_keys: '属性',
    group_attribute_keys_required: '属性不能为空',
    group_keys_at_least_one_required: '标签和属性至少需要填写一个',
    group_duration_1: '在收到告警之后，在',
    group_duration_2: '秒内收到的同组告警，一起聚合发送',
    group_duration_required: '聚合持续时间不能为空',
    default_title: '默认维度',
    default_tip: '未满足上述筛选条件，<b>按照以下维度聚合到一组进行通知</b>',
    default_duration_tip: '请注意，聚合时间间隔太大会导致告警延迟发送',
    default_duration_tip2: '聚合最大间隔不能超过 3600 秒',
    attribute_keys_map: {
      cluster: '数据源',
      cate: '数据源类型',
      group_name: '业务组',
      rule_id: '告警规则',
      rule_prod: '监控类型',
      severity: '告警等级',
      is_recovered: '是否恢复',
    },
    enable_tip: '开启后，符合规则的告警会按维度合并为一条通知 <a>使用文档</a>',
    labels_filter: {
      label_tip: '仅对满足这些标签条件的告警事件执行聚合通知。用于缩小影响范围，不配置表示不做限制。支持下拉选择已有标签键（推荐），也可手动输入',
    },
    attributes_filter: {
      label_tip: '只让匹配这些标签筛选条件的告警参与聚合，未匹配的告警不受本规则影响<br />多个条件为且关系，和下面的适用属性筛选条件也是且关系',
    },
    label_keys: {
      tip: '如果配置的是 ident，将会把 ident 相同的事件合并到一组，发送一条通知消息，常用于短信/IM 降噪',
      placeholder: '例如：ident、app，支持下拉选择已有标签键（推荐），也可手动输入',
    },
    attribute_keys: {
      tip: '如果配置的是 业务组，将会把 业务组 相同的事件合并到一组，发送一条通知消息',
      placeholder: '例如：业务组',
    },
  },
  statistics: {
    total_notify_events: '最近 {{days}} 天发送通知次数',
    total_notify_events_tip: '统计实功发送的通知次数；被<b>收敛、抑制、屏蔽</b>的事件不计入',
    escalation_events: '最近 {{days}} 天事件被升级数量',
    escalation_events_tip: '满足升级规则并被提升优先级的事件数；数量偏多通常意味着处理周期较长，需优化<b>响应SLA/升级阈值/告警压降策略</b>',
    noise_reduction_ratio: '最近 {{days}} 天降噪比',
    noise_reduction_ratio_tip: '降噪比 = <b>(1 − 实际发送通知次数 ÷ 原始告警事件数) × 100%</b>；数值越接近 <b>100%</b>，<b>降噪效果</b>越好',
  },
  tabs: {
    events: '事件列表',
    rules: '告警规则',
    sub_rules: '订阅规则',
  },
};

export default zh_CN;
