const id_ID = {
  ...
  notification_configuration: {
    ...
    severities_tip:
      'Pilih tingkat event alert mana yang diberitahukan; hanya tingkat yang dicentang yang dikirim. Bila tidak satu pun dari ketiga tingkat dicentang, media ini tidak akan cocok dengan event apa pun, sama saja dengan menonaktifkannya',
    ...
    test_with_recovery: 'Uji juga pemulihan (komentari dan tutup issue)',
    user_info: {
      user_ids: 'Penerima',
    ...
    jira: {
      project: 'Proyek',
      project_tip: 'Proyek tempat issue dibuat. Daftar diambil langsung dengan kredensial media; jika gagal dimuat, Anda dapat mengetik kunci proyek.',
      project_empty: 'Tidak ada proyek: akun di media tidak memiliki izin Browse projects',
      issue_type: 'Tipe issue',
      issue_type_tip: 'Pilih tipe issue yang sudah ada di proyek; tidak perlu membuat tipe khusus. Jika proyek memiliki tipe Alert, tipe itu dipilih secara default.',
      missing_permissions: 'Akun tidak memiliki izin berikut di proyek: {{list}}',
      required_fields: 'Field wajib tipe issue ini belum diisi di Field kustom: {{list}}; pembuatan issue akan gagal',
      on_resolve: 'Saat pulih',
      on_resolve_tip:
        'Tutup issue: komentari dan pindahkan issue ke status Selesai secara otomatis (tanpa nama transisi). Hanya komentar: issue tetap terbuka. Abaikan: issue tidak diubah.',
      on_resolve_close: 'Tutup issue',
      on_resolve_comment: 'Hanya komentar',
      on_resolve_none: 'Abaikan',
      advanced: 'Lanjutan',
      resolve_transition: 'Transisi penutupan',
      resolve_transition_tip:
        'Hanya diperlukan jika alur kerja memiliki beberapa transisi ke Selesai dan Anda ingin yang tertentu. Tidak peka huruf besar/kecil; ID numerik juga bisa.',
      resolve_transition_placeholder: 'Kosong = pilih otomatis',
      on_repeat: 'Saat notifikasi berulang',
      on_repeat_tip:
      ...
      priority_unset: 'Tidak diatur',
      priority_fill_common: 'Isi pemetaan umum',
      labels: 'Label tetap',
      labels_tip: 'Label yang ditambahkan ke setiap issue (spasi menjadi _). Label eventHash=<hash event> selalu ditambahkan untuk deduplikasi; jangan hapus di Jira.',
      labels_placeholder: 'Tekan Enter untuk menambah',
      tags_as_labels: 'Tag alert sebagai label',
      tags_as_labels_tip: 'Setiap tag alert ditulis sebagai label Jira key=value (spasi menjadi _, paling banyak 20).',
      fields: 'Field kustom',
      fields_tip:
        'Isi field kustom wajib dari tipe issue di sini. Nilai yang berupa JSON valid dikirim sebagai JSON (misalnya {"value":"prod"} untuk daftar pilihan), selain itu sebagai teks.',
  ...
  escalations: {
    ...
    title_tip:
      'Bila alert melewati durasi yang ditentukan dan belum pulih, sistem mengeskalasi notifikasinya ke kanal tertentu sesuai kondisi di bawah, agar tidak lama terabaikan. Selengkapnya di <a>dokumentasi</a>',
    ...
    labels_filter: {
      label_tip:
        'Hanya event alert yang memenuhi kondisi label ini yang dieskalasi, sehingga cakupannya menyempit; bila dikosongkan, tidak ada pembatasan. Kunci label yang sudah ada bisa dipilih dari daftar, cara yang disarankan, atau diketik sendiri',
    },
    attributes_filter: {
  ...
  notify_aggr_configs: {
    ...
    labels_filter: {
      label_tip:
        'Hanya event alert yang memenuhi kondisi label ini yang digabungkan, sehingga cakupannya menyempit; bila dikosongkan, tidak ada pembatasan. Kunci label yang sudah ada bisa dipilih dari daftar, cara yang disarankan, atau diketik sendiri',
    },
    attributes_filter: {
      label_tip:
        'Hanya alert yang cocok dengan filter label ini yang ikut diagregasi; alert lain tidak terpengaruh aturan ini<br />Antar kondisi berlaku DAN, begitu pula terhadap filter atribut di bawah',
    },
    label_keys: {
  ...
  statistics: {
    ...
    escalation_events_tip:
      'Jumlah event yang memenuhi aturan eskalasi dan naik prioritasnya. Angka yang tinggi biasanya menandakan waktu penanganan yang lama, sehingga <b>SLA respons, ambang eskalasi, atau strategi supresi alert</b> perlu diperbaiki',
    noise_reduction_ratio: 'Rasio pengurangan noise dalam {{days}} hari terakhir',
    noise_reduction_ratio_tip:
      'Rasio pengurangan noise = <b>(1 − jumlah notifikasi terkirim ÷ jumlah event alert asli) × 100%</b>. Makin dekat ke <b>100%</b>, makin baik <b>pengurangan noisenya</b>',
  },
  tabs: {
    events: 'Daftar event',
    rules: 'Aturan alert',
