const pt_BR = {
  ...
  notification_configuration: {
    ...
    severities_tip:
      'Escolha quais severidades geram notificação; apenas as marcadas são avisadas. Sem nenhuma marcada, este meio não corresponde a evento algum, o que equivale a desativá-lo',
    ...
    test_with_recovery: 'Testar também a recuperação (comentar e fechar a issue)',
    user_info: {
    ...
    jira: {
      project: 'Projeto',
      project_tip: 'O projeto onde as issues são criadas. A lista é carregada com as credenciais do meio de notificação; se não carregar, digite a chave do projeto.',
      project_empty: 'Nenhum projeto: a conta do meio de notificação não tem a permissão Browse projects',
      issue_type: 'Tipo de issue',
      issue_type_tip: 'Escolha um tipo de issue existente no projeto; não é preciso criar um específico. Se o projeto tiver o tipo Alert, ele é selecionado por padrão.',
      missing_permissions: 'A conta não tem estas permissões no projeto: {{list}}',
      required_fields: 'Campos obrigatórios deste tipo de issue não foram preenchidos em Campos personalizados: {{list}}; a criação da issue falhará',
      on_resolve: 'Na recuperação',
      on_resolve_tip:
        'Fechar issue: comentar e mover a issue automaticamente para um status Concluído (sem precisar do nome da transição). Só comentar: a issue fica aberta. Não fazer nada: a issue não é alterada.',
      on_resolve_close: 'Fechar issue',
      on_resolve_comment: 'Só comentar',
      on_resolve_none: 'Não fazer nada',
      advanced: 'Avançado',
      resolve_transition: 'Transição de fechamento',
      resolve_transition_tip:
        'Só é necessário quando o fluxo tem várias transições para Concluído e você quer uma específica. Não diferencia maiúsculas; também aceita o ID numérico.',
      resolve_transition_placeholder: 'Vazio = escolher automaticamente',
      on_repeat: 'Em notificações repetidas',
      on_repeat_tip:
      ...
      priority_unset: 'Não definir',
      priority_fill_common: 'Preencher mapeamento comum',
      labels: 'Rótulos fixos',
      labels_tip: 'Rótulos adicionados a toda issue (espaços viram _). Um rótulo eventHash=<hash do evento> é sempre adicionado para deduplicação; não o remova no Jira.',
      labels_placeholder: 'Pressione Enter para adicionar',
      tags_as_labels: 'Tags do alerta como rótulos',
      tags_as_labels_tip: 'Cada tag do alerta vira um rótulo do Jira key=value (espaços viram _, no máximo 20).',
      fields: 'Campos personalizados',
      fields_tip:
        'Preencha aqui os campos personalizados obrigatórios do tipo de issue. Um valor que seja JSON válido é enviado como JSON (por exemplo {"value":"prod"} para uma lista), caso contrário como texto.',
    ...
    discord: {
      ...
      target: 'Enviar para',
      target_tip:
        'Escolha conforme o tipo de canal do webhook:\n· Canal: publica no canal de texto do webhook (o mais comum)\n· Nova postagem no fórum: para um webhook de canal de fórum; cada notificação, incluindo a recuperação, abre uma nova postagem\n· Thread / postagem existente: publica em uma thread ou postagem existente no canal do webhook, reunindo todas as notificações; exige o ID do thread',
  ...
  escalations: {
    ...
    title_tip:
      'Passado o tempo definido sem recuperação, o sistema escalona a notificação para o canal indicado conforme as condições abaixo, evitando que o alerta fique sem acompanhamento. Detalhes na <a>documentação de uso</a>',
    ...
    labels_filter: {
      label_tip:
        'Escalona apenas os eventos que atendam a estas condições de rótulo, o que limita o alcance da regra; sem configuração, não há restrição. É possível escolher chaves existentes na lista (recomendado) ou digitá-las',
    },
    attributes_filter: {
  ...
  notify_aggr_configs: {
    ...
    labels_filter: {
      label_tip:
        'Agrega apenas os eventos que atendam a estas condições de rótulo, o que limita o alcance da regra; sem configuração, não há restrição. É possível escolher chaves existentes na lista (recomendado) ou digitá-las',
    },
    attributes_filter: {
      label_tip:
        'Apenas os alertas que atendem a estes filtros de rótulo participam da agregação; os demais não são afetados por esta regra<br />As condições são combinadas com E, inclusive com os filtros de atributo abaixo',
    },
    label_keys: {
  ...
  statistics: {
    ...
    escalation_events_tip:
      'Número de eventos que atenderam à regra de escalonamento e tiveram a prioridade elevada. Um número alto costuma indicar tempo de atendimento longo, o que pede revisar o <b>SLA de resposta, os limiares de escalonamento ou a estratégia de redução de alertas</b>',
    noise_reduction_ratio: 'Taxa de redução de ruído nos últimos {{days}} dias',
    noise_reduction_ratio_tip:
      'Taxa de redução de ruído = <b>(1 − notificações enviadas ÷ eventos de alerta originais) × 100%</b>. Quanto mais perto de <b>100%</b>, melhor a <b>redução de ruído</b>',
  },
  tabs: {
